...
/* =========================================================
   My Ship — shared presentation in js/ci-ship-presentation.js
   ========================================================= */
...
async function fetchShipFromBase44(shipName, cruiseLine = "") {
  return CiShipPresentation.fetchShip(shipName, cruiseLine);
}

function buildShipProfileFromBase44(ship, options) {
  return CiShipPresentation.buildProfile(ship, options);
}
...
async function renderTheShip() {
  ...
  const profile = buildShipProfileFromBase44(result.ship, {
    shipName,
    cruiseLine: result.ship?.cruise_line_name || cruiseLine
  });
  const cruiseLineLogo = await loadCruiseLineLogo(profile.cruiseLine || cruiseLine);
  let shipImage = await loadShipPageImage(profile.name);
