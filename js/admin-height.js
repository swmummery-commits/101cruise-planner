/**
 * Child-side Admin content-height reporter for Squarespace iframe embeds.
 * Browser global: AdminHeight
 *
 * Posts 101cruise-admin-height to allowed parent origins only (never "*").
 * Pair with js/admin-squarespace-embed.js on the Squarespace /admin page.
 */
(function (root) {
  "use strict";
  ...
  function measureHeight() {
    var root = document.getElementById("cruise-admin-app");
    if (!root) return 400;
    // Measure app content only — document scrollHeight tracks the iframe
    // viewport and can create a resize feedback loop with the parent.
    var height = Math.max(root.scrollHeight || 0, root.offsetHeight || 0);
    return Math.max(Math.ceil(height + 24), 480);
  }
  ...
  function start() {
    ...
    window.addEventListener("message", function (event) {
      ...
      if (data.type === "101cruise-request-admin-height") postHeightNow();
    });
    document.addEventListener(
      "load",
      function (event) {
        if (event.target && event.target.tagName === "IMG") schedulePost();
      },
      true
    );
    schedulePost();
    ...
    window.setTimeout(postHeightNow, 1200);
  }
