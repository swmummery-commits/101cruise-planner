-- Repair / ensure Usage & Insights table is visible to PostgREST.
-- Idempotent: safe to run if public.usage_events is missing or already present.
-- Does not change application code. Apply manually in Supabase when ready.
...
-- PostgREST only exposes relations the request role is privileged on.
-- RLS still blocks non-admin reads. Writes remain service-role only.
