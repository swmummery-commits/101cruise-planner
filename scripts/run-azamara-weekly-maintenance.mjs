#!/usr/bin/env node
/**
 * Azamara weekly maintenance — dry-run by default.
 *
 *   node scripts/run-azamara-weekly-maintenance.mjs
 *   node scripts/run-azamara-weekly-maintenance.mjs --dry-run
 *   AZAMARA_DISCOVERY_WRITE_ENABLED=true node scripts/run-azamara-weekly-maintenance.mjs --apply
 */
...
const { runAzamaraWeeklyMaintenance, AZAMARA_LINE_SLUG, AZAMARA_MAX_WEEKLY_WRITES } = require(path.join(
  root,
  "netlify/functions/lib/azamara-weekly-maintenance"
));
const { perthCalendarDate } = require(path.join(
  root,
  "netlify/functions/lib/cruise-discovery-maintenance"
));
const { redactSecrets } = require(path.join(root, "netlify/functions/lib/azamara-weekly-auth"));

const REPORT_DIR = path.join(root, "reports");

function parseArgs(argv) {
  const args = { dryRun: true, apply: false, maxWrites: AZAMARA_MAX_WEEKLY_WRITES, today: null };
...
async function main() {
  ...
  const runId = `azamara-weekly-${today.replace(/-/g, "")}-${Date.now()}`;
  ...
  const report = redactSecrets({
    ...
    quality_gate: result.simulation?.quality_gate_metrics || null,
    apply: result.apply || null
  });

  report.report_path = writeReport(
    `azamara-weekly-maintenance-${args.apply ? "apply" : "dry-run"}-${today}-${Date.now()}.json`,
    report
  );
