#!/usr/bin/env node
/**
 * Norwegian Cruise Line Phase 12 — controlled itinerary enrichment (full outstanding batch).
 *
 *   node scripts/run-norwegian-phase12-enrichment-batch.mjs --dry-run --manifest=reports/norwegian-phase12-controlled-batch-manifest-....json
 *   NORWEGIAN_ENRICHMENT_WRITE_ENABLED=true node scripts/run-norwegian-phase12-enrichment-batch.mjs --apply --confirm=NORWEGIAN-PHASE12-ENRICHMENT --manifest=...
 */
...
const APPLY_CONFIRMATION = "NORWEGIAN-PHASE12-ENRICHMENT";
...
async function loadPopulation(sb, coreManifestPath) {
  ...
  const phase12 = JSON.parse(fs.readFileSync(resolvedPath, "utf8"));
  ...
  const matches = [];
  const missing = [];
  for (const entry of phase12.entries) {
  ...
  const extras = genuine.filter((r) => !phase12.entries.some((e) => e.official_sailing_id === r.official_sailing_id));
  ...
  const expectedCount = phase12.entries.length;
  return {
    core_manifest_path: resolvedPath,
    manifest_count: expectedCount,
    expected_count: expectedCount,
    db_genuine_count: genuine.length,
    exact_match: missing.length === 0 && matches.length === expectedCount,
    missing,
...
async function verifyPostWrite(sb, population, dryRunManifest, legacySnapshots) {
  ...
  return {
    ...
    passed: genuine.length >= population.expected_count && mismatches.length === 0 && legacyChanged.length === 0
  };
}
...
async function main() {
  ...
  if (!args.manifestPath && !args.verifyOnly && !args.idempotencyOnly) {
    throw new Error("--manifest=path to Phase 12 core manifest is required");
  }
  const sb = createMaintenanceSupabase(root);
  const today = perthCalendarDate();
  const runId =
    args.runId || `norwegian-phase12-enrichment-${today}-${new Date().toISOString().replace(/[:.]/g, "-")}`;
  ...
  if (summary.tests_pre.ncl.passed < 131) throw new Error(`NCL tests regressed: ${summary.tests_pre.ncl.passed}`);
  ...
  if (!summary.population.exact_match) {
    throw new Error(
      `Phase 12 identity set mismatch: missing=${summary.population.missing.length} count=${summary.population.matches.length}`
    );
  }
  ...
  const existingDryRunPath = args.runId
    ? path.join(REPORT_DIR, `norwegian-phase12-enrichment-dry-run-${args.runId}.json`)
    : null;
  ...
  if ((args.verifyOnly || args.idempotencyOnly) && existingDryRunPath && fs.existsSync(existingDryRunPath)) {
    ...
    const existingApplyPath = path.join(REPORT_DIR, `norwegian-phase12-enrichment-apply-${args.runId}.json`);
    ...
    summary.pre_write_snapshot_path = writeReport(`norwegian-phase12-enrichment-pre-write-snapshot-${today}.json`, preWriteSnapshot);

    const phase12EnrichmentManifest = {
    ...
    summary.phase12_enrichment_manifest_path = writeReport(
      `norwegian-phase12-enrichment-manifest-${runId}.json`,
      phase12EnrichmentManifest
    );
    ...
    dryRunManifest = await enrichment.buildDryRunManifest(
      ...
      { fetchDelayMs: 300, supabase: sb }
    );
    summary.dry_run_path = writeReport(`norwegian-phase12-enrichment-dry-run-${runId}.json`, dryRunManifest);
    summary.dry_run = {
      page_stats: dryRunManifest.page_stats,
      port_totals: dryRunManifest.port_totals,
      outcome_counts: dryRunManifest.outcome_counts,
    ...
    if (args.apply || args.full) {
      ...
      try {
        ...
        summary.apply_path = writeReport(`norwegian-phase12-enrichment-apply-${runId}.json`, summary.apply);
  ...
  if (args.verify || args.full || args.verifyOnly) {
    ...
    summary.post_write_path = writeReport(`norwegian-phase12-enrichment-post-write-verify-${runId}.json`, summary.post_write);
  ...
  summary.final_report_path = writeReport(`norwegian-phase12-enrichment-final-report-${runId}.json`, summary);
