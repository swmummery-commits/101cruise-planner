#!/usr/bin/env node
/**
 * Norwegian Cruise Line Phase 6 — 50-voyage controlled production catch-up.
 *
 *   node scripts/run-norwegian-phase6-controlled-batch.mjs --preflight
 *   node scripts/run-norwegian-phase6-controlled-batch.mjs --full --dry-run-only
 *   NORWEGIAN_DISCOVERY_WRITE_ENABLED=true node scripts/run-norwegian-phase6-controlled-batch.mjs --full --apply --confirm=NORWEGIAN-PHASE6-CONTROLLED-BATCH
 */
...
const PHASE5C_COMMIT = "629268c19d6a0d9885ef0d608c25ac967f82a1c0";
...
const MAX_WRITES = 50;
const APPLY_CONFIRMATION = "NORWEGIAN-PHASE6-CONTROLLED-BATCH";
...
async function checkRepositoryBaseline() {
  ...
  const phase5cOnMain = runGit(`git merge-base --is-ancestor ${PHASE5C_COMMIT} origin/main && echo yes || echo no`);
  ...
  return {
    ...
    phase5c_commit: PHASE5C_COMMIT,
    phase5c_on_origin_main: phase5cOnMain.trim() === "yes"
  };
}
...
async function runLiveSnapshot(sb, line, ships, today) {
  ...
  const report = {
    ...
    mode: "norwegian_phase6_live_source_snapshot",
    today,
  ...
  const pathOut = writeReport(`norwegian-phase6-source-snapshot-${today}.json`, {
    ...report,
    products: undefined,
...
async function productionBaseline(sb, lineId) {
  ...
  const pathOut = writeReport("norwegian-phase6-production-baseline-pre-write.json", report);
  return { report, report_path: pathOut, legacy_snapshots: report.legacy_row_snapshots };
}

function defaultBatchId(today) {
  return `norwegian-phase6-${today}-${new Date().toISOString().replace(/[:.]/g, "-")}`;
}
...
async function buildFrozenManifest(simulation, sb, line, batchId, sourceTimestamp) {
  ...
  const manifest = await buildManifestFromEntries({
    ...
    batchId,
    sourceTimestamp,
    mode: "norwegian_phase6_controlled_batch",
    phase: "phase6_controlled_import",
  ...
  const filename = `norwegian-phase6-controlled-batch-manifest-${batchId}.json`;
...
async function main() {
  ...
  if (args.preflight || args.full) {
    ...
    if (!summary.repo.phase5c_on_origin_main) {
      console.error(JSON.stringify({ error: "Phase 5C commit not on origin/main", repo: summary.repo }, null, 2));
      process.exit(2);
    }
    ...
    writeReport("norwegian-phase6-preflight-tests-pre.json", summary.tests_pre);
  ...
  let baseline = null;
  if (args.baseline || args.manifest || args.apply || args.full || args.verify || args.idempotency) {
  ...
  if (args.manifest || args.dryRun || args.apply || args.full) {
    ...
    writeReport(`norwegian-phase6-pre-apply-checkpoint-${manifest.batch_id}.json`, {
    ...
    writeReport(`norwegian-phase6-dry-run-${manifest.batch_id}.json`, {
      batch_id: manifest.batch_id,
      dry_run_gate: gate,
  ...
  if (args.apply && !args.dryRunOnly) {
    ...
    try {
      ...
      writeReport(`norwegian-phase6-apply-${manifest.batch_id}.json`, applyResult);
  ...
  if (args.verify && manifest && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase6-post-write-verify-${manifest.batch_id}.json`, summary.post_write_verification);
  ...
  if (args.idempotency && manifest && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase6-idempotency-${manifest.batch_id}.json`, summary.idempotency);
  ...
  if ((args.verify || args.idempotency || args.full) && simulation && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase6-reconciliation-${manifest?.batch_id || runId}.json`, summary.reconciliation);
  }

  if ((args.apply || args.full) && !args.dryRunOnly) {
    summary.tests_post = await runPreflightTests();
    writeReport("norwegian-phase6-preflight-tests-post.json", summary.tests_post);
  ...
  if (fs.existsSync(PHASE4_MANIFEST) && (args.verify || args.full) && !args.dryRunOnly) {
    const phase4 = JSON.parse(fs.readFileSync(PHASE4_MANIFEST, "utf8"));
    const indexes = await indexExistingNorwegianRecords(sb, line.id);
    const regressions = [];
    for (const entry of phase4.entries || []) {
      const row = indexes.byOfficial.get(entry.official_sailing_id);
      if (!row) regressions.push({ official_sailing_id: entry.official_sailing_id, issue: "missing" });
      else if (row.status !== "match_required") regressions.push({ official_sailing_id: entry.official_sailing_id, issue: "status_changed", status: row.status });
      else if (row.destination_id !== entry.resolved_destination_id && entry.resolved_destination_id) {
        regressions.push({ official_sailing_id: entry.official_sailing_id, issue: "destination_changed" });
      }
    }
    summary.phase4_regression = { checked: phase4.entries?.length || 0, regressions, passed: regressions.length === 0 };
  }

  const finalReportPath = writeReport(`norwegian-phase6-final-report-${runId}.json`, summary);
