#!/usr/bin/env node
/**
 * Silversea Phase 4C — apply canonical ports + existing-port alias/sync fixes.
 *
 *   node scripts/apply-silversea-phase4c-port-batch.mjs --dry-run
 *   node scripts/apply-silversea-phase4c-port-batch.mjs --apply
 */
...
function runId() {
  return `silversea-phase4c-port-${new Date().toISOString().replace(/[:.]/g, "-")}`;
}
...
function planAliasWrites(csvText) {
  ...
  const writes = [];
  const skipped = [];
  for (const spec of PHASE4C_EXISTING_PORT_ALIASES) {
...
function resolveForVerification(alias, spec) {
  ...
  if (
    ...
    alias === spec.silversea_source_name &&
    PHASE4C_SILVERSEA_ADAPTER_ALIASES.some((row) => row.source_label === alias)
  ) {
    resolution = adapter.resolveSilverseaPort(alias, "silversea_phase4c_verify");
  }
  return resolution;
}
...
function verifyAll(specs, aliasWrites) {
  ...
  for (const mapping of PHASE4C_SILVERSEA_ADAPTER_ALIASES) {
    const resolution = adapter.resolveSilverseaPort(mapping.source_label, "silversea_phase4c_verify");
    checks.push({
...
async function main() {
  ...
  for (const [index, spec] of PHASE4C_CANONICAL_PORT_CREATES.entries()) {
  ...
  if (proposed.length > 25) {
    throw new Error(`Proposed new canonical ports ${proposed.length} exceed Phase 4C limit of 25`);
  }
  ...
  if (APPLY && (aliasPlan.writes.length || proposed.length || csvSyncOnly.length)) {
    ...
    for (const spec of proposed) {
      const payload = {
        ...
        status: "verified",
        source: "silversea:phase4c_canonical_ports",
  ...
  if (APPLY) {
    report.verification = verifyAll(PHASE4C_CANONICAL_PORT_CREATES, aliasPlan.writes);
  ...
  const rollbackPath = path.join(REPORT_DIR, `silversea-phase4c-port-rollback-${id}.json`);
