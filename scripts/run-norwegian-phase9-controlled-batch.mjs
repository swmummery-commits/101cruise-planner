#!/usr/bin/env node
/**
 * Norwegian Cruise Line Phase 9 — controlled 100-voyage production catch-up (staged 1 + 24 + 75).
 *
 *   node scripts/run-norwegian-phase9-controlled-batch.mjs --preflight
 *   node scripts/run-norwegian-phase9-controlled-batch.mjs --full --dry-run-only
 *   NORWEGIAN_DISCOVERY_WRITE_ENABLED=true node scripts/run-norwegian-phase9-controlled-batch.mjs --full --confirm=NORWEGIAN-PHASE9-CONTROLLED-BATCH
 */
...
const PHASE8_COMMIT = "60ff72c60eb75d4848f50b49c7f0307f568cf643";
...
const MIN_DISTINCT_SHIPS = 18;
const APPLY_CONFIRMATION = "NORWEGIAN-PHASE9-CONTROLLED-BATCH";
...
async function checkRepositoryBaseline() {
  ...
  const phase8OnMain = runGit(`git merge-base --is-ancestor ${PHASE8_COMMIT} origin/main && echo yes || echo no`);
  ...
  return {
    ...
    phase8_on_origin_main: phase8OnMain.trim() === "yes"
  };
}
...
async function checkNetlifyDeploy() {
  ...
  try {
    ...
    if (!deployedSha) {
      ...
      const localMainHasPhase8 =
        runGit(`git merge-base --is-ancestor ${PHASE8_COMMIT} HEAD && echo yes || echo no`).trim() === "yes";
      ...
      result.phase8_contained = localMainHasPhase8;
      result.passed = localMainHasPhase6a && localMainHasPhase8;
      result.error = result.passed ? null : "deployed_commit_ref_missing";
      result.note = "Published deploy lacks commit_ref; verified Phase 6A/8 fixes on local main HEAD for script writes";
      return result;
    }

    const phase6aAncestor = runGit(`git merge-base --is-ancestor ${PHASE6A_COMMIT} ${deployedSha} && echo yes || echo no`);
    const phase8Ancestor = runGit(`git merge-base --is-ancestor ${PHASE8_COMMIT} ${deployedSha} && echo yes || echo no`);
    result.deploy_id = deploy.id;
    ...
    result.phase8_contained = phase8Ancestor.trim() === "yes";
    result.passed = result.phase6a_contained && result.phase8_contained;
  } catch (error) {
...
async function runLiveSnapshot(sb, line, ships, today) {
  ...
  const report = {
    ...
    mode: "norwegian_phase9_live_source_snapshot",
    today,
  ...
  const pathOut = writeReport(`norwegian-phase9-source-snapshot-${today}.json`, {
    ...report,
    products: undefined,
...
async function productionBaseline(sb, lineId) {
  ...
  const pathOut = writeReport("norwegian-phase9-production-baseline-pre-write.json", report);
  return {
    report,
    report_path: pathOut,
...
function defaultBatchId(today) {
  return `norwegian-phase9-${today}-${new Date().toISOString().replace(/[:.]/g, "-")}`;
}
...
async function buildFrozenManifest(simulation, sb, line, batchId, sourceTimestamp) {
  ...
  if (selection.distinct_ships < MIN_DISTINCT_SHIPS) {
    throw new Error(
      `Distinct ship count ${selection.distinct_ships} below minimum ${MIN_DISTINCT_SHIPS} for Phase 9 batch`
    );
  }
  ...
  const manifest = await buildManifestFromEntries({
    ...
    batchId,
    sourceTimestamp,
    mode: "norwegian_phase9_controlled_batch",
    phase: "phase9_controlled_import",
  ...
  const filename = `norwegian-phase9-controlled-batch-manifest-${batchId}.json`;
...
async function verifyCanaryReadBack(sb, lineId, entry) {
  ...
  return {
    ...
    phase9_canary_status_after_insert: canaryStatus,
    checks,
    passed: Boolean(row) && (rows || []).length === 1 && Object.values(checks).every(Boolean)
  };
}

async function verifyPhase9BatchStatus(sb, lineId, manifest, { upToPosition = MAX_WRITES } = {}) {
  ...
  return {
    ...
    phase9_records: officialIds.length,
...
async function main() {
  ...
  const willWrite = (args.stage1Apply || args.stage2Apply || args.stage3Apply) && !args.dryRunOnly;
  ...
  if (args.preflight || args.full) {
    ...
    if (!summary.repo.phase8_on_origin_main) {
      console.error(JSON.stringify({ error: "Phase 8 commit not on origin/main", repo: summary.repo }, null, 2));
      process.exit(2);
    }

    summary.deploy = await checkNetlifyDeploy();
    writeReport("norwegian-phase9-netlify-deploy-check.json", summary.deploy);
    ...
    writeReport("norwegian-phase9-preflight-tests-pre.json", summary.tests_pre);
  ...
  let simulation = null;
  if (args.snapshot || args.manifest || args.dryRun || args.stage1Apply || args.stage2Apply || args.stage3Apply || args.full) {
  ...
  let baseline = null;
  if (args.baseline || args.manifest || args.stage1Apply || args.stage2Apply || args.stage3Apply || args.full || args.verify || args.idempotency) {
  ...
  let manifest = null;
  if (args.manifest || args.dryRun || args.stage1Apply || args.stage2Apply || args.stage3Apply || args.full) {
    ...
    writeReport(`norwegian-phase9-pre-apply-checkpoint-${manifest.batch_id}.json`, {
    ...
    writeReport(`norwegian-phase9-dry-run-${manifest.batch_id}.json`, {
      batch_id: manifest.batch_id,
      dry_run_gate: gate,
  ...
  if (args.stage1Apply && !args.dryRunOnly) {
    ...
    try {
      ...
      writeReport(`norwegian-phase9-stage1-${manifest.batch_id}.json`, {
        ...stage1ApplyResult,
        stage1_entry: {
      ...
      writeReport(`norwegian-phase9-stage1-verify-${manifest.batch_id}.json`, summary.stage1_read_back);
      ...
      summary.phase9_stage1_status_after_insert = "match_required";
    } finally {
  ...
  if (args.stage2Apply && !args.dryRunOnly) {
    ...
    try {
      ...
      writeReport(`norwegian-phase9-stage2-${manifest.batch_id}.json`, stage2ApplyResult);
      ...
      summary.stage2_status = await verifyPhase9BatchStatus(sb, line.id, manifest, {
        upToPosition: STAGE2_CUMULATIVE
      });
      writeReport(`norwegian-phase9-stage2-status-${manifest.batch_id}.json`, summary.stage2_status);
  ...
  if (args.stage3Apply && !args.dryRunOnly) {
    ...
    try {
      const stage2Checkpoint = await verifyPhase9BatchStatus(sb, line.id, manifest, {
        upToPosition: STAGE2_CUMULATIVE
      });
      ...
      writeReport(`norwegian-phase9-stage3-${manifest.batch_id}.json`, stage3ApplyResult);
      ...
      summary.stage3_status = await verifyPhase9BatchStatus(sb, line.id, manifest, { upToPosition: MAX_WRITES });
      writeReport(`norwegian-phase9-stage3-status-${manifest.batch_id}.json`, summary.stage3_status);
      if (!summary.stage3_status.passed) {
        throw new Error("Stage 3 status verification failed — active or non-match_required rows detected");
      }
  ...
  if (args.verify && manifest && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase9-post-write-verify-${manifest.batch_id}.json`, summary.post_write_verification);
  ...
  if (args.idempotency && manifest && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase9-idempotency-${manifest.batch_id}.json`, summary.idempotency);
  ...
  if ((args.verify || args.idempotency || args.full) && simulation && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase9-reconciliation-${manifest?.batch_id || runId}.json`, summary.reconciliation);
    ...
    if (baseline?.pre_batch_genuine_official_ids?.length) {
      ...
      writeReport(`norwegian-phase9-phase6-regression-${manifest?.batch_id || runId}.json`, summary.phase6_regression);
  ...
  if ((args.stage1Apply || args.stage2Apply || args.stage3Apply || args.full) && !args.dryRunOnly) {
    summary.tests_post = await runPreflightTests();
    writeReport("norwegian-phase9-preflight-tests-post.json", summary.tests_post);
  ...
  summary.production_voyage_writes =
    args.stage1Apply && args.stage2Apply && args.stage3Apply && !args.dryRunOnly ? MAX_WRITES : 0;
  summary.new_publicly_active_ncl_voyages = 0;
  summary.temporary_activation_during_phase9 = 0;
  summary.ncl_weekly_cron_enabled = false;

  const finalReportPath = writeReport(`norwegian-phase9-final-report-${runId}.json`, summary);
