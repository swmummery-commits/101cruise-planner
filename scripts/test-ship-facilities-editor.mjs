...
assert.match(plannerJs, /CiShipPresentation/);
assert.doesNotMatch(plannerJs, /passenger decks/);
...
assert.match(adminJs, /bindCiShipFeatureEditorsAfterRender/);
assert.match(adminJs, /bindCiShipFeatureEditorsAfterRender\(\)/);
assert.doesNotMatch(adminJs, /setTimeout\(function \(\) \{\s*bindCiShipFeatureList/);
assert.doesNotMatch(adminJs, /setTimeout\(function \(\) \{\s*rebuildCiSpecialtyFeaturesDom/);
assert.match(read("js/ci-ship-feature-admin.js"), /closeAllIconMenus/);
assert.match(read("js/ci-ship-feature-admin.js"), /is-open/);
assert.match(read("css/admin.css"), /\.ci-ship-feature-icon-picker\.is-open/);
