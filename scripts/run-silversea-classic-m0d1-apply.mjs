#!/usr/bin/env node
/**
 * Silversea Classic Phase M0D1 — first controlled itinerary_ports backfill (200 of 599).
 *
 *   node scripts/run-silversea-classic-m0d1-apply.mjs --preflight
 *   SILVERSEA_DISCOVERY_WRITE_ENABLED=true node scripts/run-silversea-classic-m0d1-apply.mjs \
 *     --apply --confirm=SILVERSEA-CLASSIC-M0D1-ITINERARY-PORTS-BACKFILL
 */
...
export const M0D1_RUNNER_PATH = "scripts/run-silversea-classic-m0d1-apply.mjs";
export const M0D1_USES_HARDENED_RUNNER = true;
...
function assertApplyAllowed(args) {
  ...
  if (args.confirm !== M0D1_APPLY_CONFIRMATION_TOKEN) {
    throw new Error("m0d1_apply_confirmation_required");
  }
...
export async function runSilverseaClassicM0d1(options = {}) {
  ...
  fs.writeFileSync(M0D1_FIXTURE_PATH, `${JSON.stringify(m0d1Fixture, null, 2)}\n`);

  const repairRows = m0d1Fixture.rows;
  const targetUuids = new Set(repairRows.map((r) => r.production_uuid));
  const m0d2Uuids = new Set(partition.batches.m0d2.rows.map((r) => r.production_uuid));
  const m0d3Uuids = new Set(partition.batches.m0d3.rows.map((r) => r.production_uuid));
  const dryRun = dryRunClassicItineraryPortsBackfill(m0d1Fixture);
  ...
  const runId = options.runId || `silversea-classic-m0d1-itinerary-ports-${startedAt.replace(/[:.]/g, "-")}`;
  ...
  rollbackManifest.operation = M0D1_OPERATION;

  let applyReport = buildApplyReportLifecycle({
    runId,
    createdAt: startedAt,
    fixturePath: M0D1_BACKFILL_FIXTURE,
    operation: M0D1_OPERATION,
    lineSlug: adapter.LINE_SLUG,
    expectedInserts: 0,
    productionBefore: countsBefore
  });
  applyReport.phase = "M0D1";
  ...
  applyReport.m0d1_runner_path = M0D1_RUNNER_PATH;
    ...
    hardenedResult = await executeHardenedControlledProductionApply(
      ...
      },
      {
        ...
        verifyUnderLock: async ({ writeResult: wr, lockMeta }) => {
          ...
          protection = {
            m0d2_unchanged: m0d2Check.ok,
            m0d2_issues: m0d2Check.issues.slice(0, 5),
          ...
          const allOk =
            verification.ok &&
            classicAudit.exact_match === EXPECTED_BATCH_SIZE &&
            classicAudit.remaining_repair_candidates === 399 &&
  ...
  const report = {
    phase: args.apply ? "m0d1_apply" : "m0d1_preflight",
    run_id: runId,
    m0d1_runner_path: M0D1_RUNNER_PATH,
    m0d1_uses_hardened_runner: true,
    ...
    remaining_classic_repairs: args.apply && classicAudit ? classicAudit.remaining_repair_candidates : 399,
    expedition_data_shape_complete: true,
    weekly_maintenance: "NOT ENABLED",
    m1_authorised: false,
    next_phase:
      args.apply && verification?.ok
  ...
  const reportPath = path.join(REPORT_DIR, `silversea-classic-m0d1-${runId}.json`);
...
async function main() {
  try {
    const report = await runSilverseaClassicM0d1();
    console.log(
      JSON.stringify(
        {
          ok: !report.blocked,
    ...
    if (report.phase === "m0d1_apply" && report.production_writes.itinerary_ports_updates !== EXPECTED_BATCH_SIZE) {
      process.exit(1);
    }
  } catch (err) {
