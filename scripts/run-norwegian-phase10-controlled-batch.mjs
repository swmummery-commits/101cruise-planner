#!/usr/bin/env node
/**
 * Norwegian Cruise Line Phase 10 — controlled 200-voyage production catch-up (staged 1 + 24 + 75 + 100).
 *
 *   node scripts/run-norwegian-phase10-controlled-batch.mjs --preflight
 *   node scripts/run-norwegian-phase10-controlled-batch.mjs --full --dry-run-only
 *   NORWEGIAN_DISCOVERY_WRITE_ENABLED=true node scripts/run-norwegian-phase10-controlled-batch.mjs --full --confirm=NORWEGIAN-PHASE10-CONTROLLED-BATCH
 */
...
const PHASE9_COMMIT = "6f56788daec06ad4a83f6931ff270143038a966f";
...
const MIN_DISTINCT_SHIPS = 20;
const APPLY_CONFIRMATION = "NORWEGIAN-PHASE10-CONTROLLED-BATCH";
...
async function checkRepositoryBaseline() {
  ...
  const phase9OnMain = runGit(`git merge-base --is-ancestor ${PHASE9_COMMIT} origin/main && echo yes || echo no`);
  ...
  return {
    ...
    phase9_on_origin_main: phase9OnMain.trim() === "yes"
  };
}
...
async function checkNetlifyDeploy() {
  ...
  try {
    ...
    if (!deployedSha) {
      ...
      const localMainHasPhase9 =
        runGit(`git merge-base --is-ancestor ${PHASE9_COMMIT} HEAD && echo yes || echo no`).trim() === "yes";
      ...
      result.phase9_contained = localMainHasPhase9;
      result.passed = localMainHasPhase6a && localMainHasPhase9;
    ...
    const phase9Ancestor = runGit(`git merge-base --is-ancestor ${PHASE9_COMMIT} ${deployedSha} && echo yes || echo no`);
    result.deploy_id = deploy.id;
    ...
    result.phase9_contained = phase9Ancestor.trim() === "yes";
    result.passed = result.phase6a_contained && result.phase9_contained;
  } catch (error) {
...
async function runLiveSnapshot(sb, line, ships, today) {
  ...
  const report = {
    ...
    mode: "norwegian_phase10_live_source_snapshot",
    today,
  ...
  const pathOut = writeReport(`norwegian-phase10-source-snapshot-${today}.json`, {
    ...report,
    products: undefined,
...
async function productionBaseline(sb, lineId) {
  ...
  const pathOut = writeReport("norwegian-phase10-production-baseline-pre-write.json", report);
  return {
    report,
    report_path: pathOut,
...
function defaultBatchId(today) {
  return `norwegian-phase10-${today}-${new Date().toISOString().replace(/[:.]/g, "-")}`;
}
...
async function buildFrozenManifest(simulation, sb, line, batchId, sourceTimestamp) {
  ...
  const selection = selectPhase6BatchProducts(simulation.products, {
    ...
    minDistinctShips: MIN_DISTINCT_SHIPS
  });
  const selected = selection.selected;
  if (selected.length !== MAX_WRITES) {
    throw new Error(`Could only select ${selected.length}/${MAX_WRITES} eligible voyages`);
  }
  if (selection.distinct_ships < MIN_DISTINCT_SHIPS) {
    throw new Error(
      `Distinct ship count ${selection.distinct_ships} below minimum ${MIN_DISTINCT_SHIPS} for Phase 10 batch`
    );
  }
  ...
  const manifest = await buildManifestFromEntries({
    ...
    batchId,
    sourceTimestamp,
    mode: "norwegian_phase10_controlled_batch",
    phase: "phase10_controlled_import",
  ...
  const filename = `norwegian-phase10-controlled-batch-manifest-${batchId}.json`;
...
async function verifyCanaryReadBack(sb, lineId, entry) {
  ...
  return {
    ...
    phase10_canary_status_after_insert: canaryStatus,
    phase10_canary_destination_id_preserved:
...
async function verifyPhase10BatchStatus(sb, lineId, manifest, { upToPosition = MAX_WRITES } = {}) {
  ...
  return {
    ...
    phase10_records: officialIds.length,
...
function assertWriteGates(args, summary) {
  ...
  if (!summary.deploy?.phase9_contained) {
    throw new Error("Production deploy does not contain Phase 9 commit (6f56788) — aborting writes");
  }
...
async function main() {
  ...
  const willWrite = (args.stage1Apply || args.stage2Apply || args.stage3Apply || args.stage4Apply) && !args.dryRunOnly;
  ...
  if (args.preflight || args.full) {
    ...
    if (!summary.repo.phase9_on_origin_main) {
    ...
    writeReport("norwegian-phase10-netlify-deploy-check.json", summary.deploy);
    ...
    writeReport("norwegian-phase10-preflight-tests-pre.json", summary.tests_pre);
  ...
  let simulation = null;
  if (args.snapshot || args.manifest || args.dryRun || args.stage1Apply || args.stage2Apply || args.stage3Apply || args.stage4Apply || args.full) {
  ...
  let baseline = null;
  if (args.baseline || args.manifest || args.stage1Apply || args.stage2Apply || args.stage3Apply || args.stage4Apply || args.full || args.verify || args.idempotency) {
  ...
  let manifest = null;
  if (args.manifest || args.dryRun || args.stage1Apply || args.stage2Apply || args.stage3Apply || args.stage4Apply || args.full) {
    ...
    writeReport(`norwegian-phase10-pre-apply-checkpoint-${manifest.batch_id}.json`, {
    ...
    writeReport(`norwegian-phase10-dry-run-${manifest.batch_id}.json`, {
      batch_id: manifest.batch_id,
      dry_run_gate: gate,
  ...
  if (args.stage1Apply && !args.dryRunOnly) {
    ...
    try {
      ...
      writeReport(`norwegian-phase10-stage1-${manifest.batch_id}.json`, {
        ...stage1ApplyResult,
        stage1_entry: {
      ...
      writeReport(`norwegian-phase10-stage1-verify-${manifest.batch_id}.json`, summary.stage1_read_back);
      ...
      summary.phase10_stage1_status_after_insert = "match_required";
    } finally {
  ...
  if (args.stage2Apply && !args.dryRunOnly) {
    ...
    try {
      ...
      writeReport(`norwegian-phase10-stage2-${manifest.batch_id}.json`, stage2ApplyResult);
      ...
      summary.stage2_status = await verifyPhase10BatchStatus(sb, line.id, manifest, {
        upToPosition: STAGE2_CUMULATIVE
      });
      writeReport(`norwegian-phase10-stage2-status-${manifest.batch_id}.json`, summary.stage2_status);
  ...
  if (args.stage3Apply && !args.dryRunOnly) {
    ...
    try {
      const stage2Checkpoint = await verifyPhase10BatchStatus(sb, line.id, manifest, {
        upToPosition: STAGE2_CUMULATIVE
      });
      ...
      writeReport(`norwegian-phase10-stage3-${manifest.batch_id}.json`, stage3ApplyResult);
      ...
      summary.stage3_status = await verifyPhase10BatchStatus(sb, line.id, manifest, {
        upToPosition: STAGE3_CUMULATIVE
      });
      writeReport(`norwegian-phase10-stage3-status-${manifest.batch_id}.json`, summary.stage3_status);
  ...
  if (args.stage4Apply && !args.dryRunOnly) {
    ...
    try {
      const stage3Checkpoint = await verifyPhase10BatchStatus(sb, line.id, manifest, {
        upToPosition: STAGE3_CUMULATIVE
      });
      ...
      writeReport(`norwegian-phase10-stage4-${manifest.batch_id}.json`, stage4ApplyResult);
      ...
      summary.stage4_status = await verifyPhase10BatchStatus(sb, line.id, manifest, { upToPosition: MAX_WRITES });
      writeReport(`norwegian-phase10-stage4-status-${manifest.batch_id}.json`, summary.stage4_status);
  ...
  if (args.verify && manifest && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase10-post-write-verify-${manifest.batch_id}.json`, summary.post_write_verification);
  ...
  if (args.idempotency && manifest && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase10-idempotency-${manifest.batch_id}.json`, summary.idempotency);
  ...
  if ((args.verify || args.idempotency || args.full) && simulation && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase10-reconciliation-${manifest?.batch_id || runId}.json`, summary.reconciliation);
    ...
    if (baseline?.pre_batch_genuine_official_ids?.length) {
      ...
      writeReport(`norwegian-phase10-phase6-regression-${manifest?.batch_id || runId}.json`, summary.phase6_regression);
  ...
  if ((args.stage1Apply || args.stage2Apply || args.stage3Apply || args.stage4Apply || args.full) && !args.dryRunOnly) {
    summary.tests_post = await runPreflightTests();
    writeReport("norwegian-phase10-preflight-tests-post.json", summary.tests_post);
  ...
  summary.production_voyage_writes =
    args.stage1Apply && args.stage2Apply && args.stage3Apply && args.stage4Apply && !args.dryRunOnly
      ? MAX_WRITES
      : 0;
  summary.new_core_import_destination_id_nulls = 0;
  summary.new_publicly_active_ncl_voyages = 0;
  summary.temporary_activation_during_phase10 = 0;
  summary.ncl_weekly_cron_enabled = false;

  const finalReportPath = writeReport(`norwegian-phase10-final-report-${runId}.json`, summary);
