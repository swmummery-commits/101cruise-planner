#!/usr/bin/env node
/**
 * Silversea Phase 4B — apply canonical ports + existing-port alias/sync fixes.
 *
 *   node scripts/apply-silversea-phase4b-port-batch.mjs --dry-run
 *   node scripts/apply-silversea-phase4b-port-batch.mjs --apply
 */
...
function runId() {
  return `silversea-phase4b-port-${new Date().toISOString().replace(/[:.]/g, "-")}`;
}
...
function planAliasWrites(csvText) {
  ...
  const writes = [];
  const skipped = [];
  for (const spec of PHASE4B_EXISTING_PORT_ALIASES) {
...
async function main() {
  ...
  for (const [index, spec] of PHASE4B_CANONICAL_PORT_CREATES.entries()) {
  ...
  if (proposed.length > 25) {
    throw new Error(`Proposed new canonical ports ${proposed.length} exceed Phase 4B limit of 25`);
  }
  ...
  if (APPLY && (aliasPlan.writes.length || proposed.length || csvSyncOnly.length)) {
    ...
    for (const spec of proposed) {
      const payload = {
        ...
        status: "verified",
        source: "silversea:phase4b_canonical_ports",
  ...
  if (APPLY) {
    report.verification = verifyAll(PHASE4B_CANONICAL_PORT_CREATES, aliasPlan.writes);
  ...
  const rollbackPath = path.join(REPORT_DIR, `silversea-phase4b-port-rollback-${id}.json`);
