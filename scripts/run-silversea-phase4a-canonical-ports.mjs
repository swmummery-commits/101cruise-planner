#!/usr/bin/env node
/**
 * Silversea Phase 4A — canonical port creation analysis and eligibility recheck.
 *
 *   node scripts/run-silversea-phase4a-canonical-ports.mjs
 *   node scripts/run-silversea-phase4a-canonical-ports.mjs --after-apply --write-frozen
 */
...
const { groupUnresolvedPortOccurrences } = require(path.join(root, "netlify/functions/lib/silversea-port-remediation"));
const { PHASE4A_CANONICAL_PORT_CREATES } = require(path.join(
  root,
  "netlify/functions/lib/silversea-canonical-port-batch"
));
...
async function main() {
  ...
  const endpointBlockers = grouped.filter((g) => g.roles.includes("embark") || g.roles.includes("disembark"));
  ...
  const report = {
    ...
    endpoint_blockers: endpointBlockers,
    prioritised_missing_ports: grouped.slice(0, 50),
    proposed_canonical_ports: PHASE4A_CANONICAL_PORT_CREATES,
  ...
  const reportPath = path.join(REPORT_DIR, `silversea-phase4a-${suffix}-${stamp}.json`);
  fs.writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`);

  let frozenPath = null;
  if (WRITE_FROZEN && AFTER_APPLY && newEligibleCount > 0) {
    if (!fs.existsSync(FIXTURE_DIR)) fs.mkdirSync(FIXTURE_DIR, { recursive: true });
    frozenPath = path.join(FIXTURE_DIR, "phase4a-frozen-eligible.json");
    fs.writeFileSync(
      frozenPath,
      `${JSON.stringify(
        {
          phase: "phase4a_frozen_eligible",
