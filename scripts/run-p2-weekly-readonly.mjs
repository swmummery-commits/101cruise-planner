#!/usr/bin/env node
/**
 * P2 production-equivalent READ-ONLY weekly maintenance into the central ledger.
 * Never enables write flags. Never --apply.
 *
 *   node scripts/run-p2-weekly-readonly.mjs --line=hal
 *   node scripts/run-p2-weekly-readonly.mjs --line=norwegian
 */
...
function loadLineSpec(line) {
  ...
  if (line === "norwegian") {
    const { runNorwegianWeeklyBackgroundMaintenance } = require(path.join(
      root,
      "netlify/functions/lib/norwegian-weekly-maintenance-dispatch"
    ));
    return { kind: "norwegian", runNorwegianWeeklyBackgroundMaintenance };
  }
  const {
...
function parseArgs(argv) {
  ...
  if (!ALLOWED.includes(line)) {
    throw new Error(`--line must be ${ALLOWED.join(", ")}`);
  }
  const pass = String(argv.find((arg) => arg.startsWith("--pass="))?.slice("--pass=".length) || "1");
  return { line, pass };
}
...
function compact(summary = {}) {
  return {
    ...
    eligible_total: summary.eligible_total ?? summary.eligible_ocean ?? null,
    ...
    proposed_review_updates: summary.proposed_review_updates ?? summary.identity_review_count ?? null,
...
async function main() {
  ...
  const { line, pass } = parseArgs(process.argv.slice(2));
  const spec = loadLineSpec(line);
  const sb = createMaintenanceSupabase(root);
  const triggerType = "p2_readonly";
  ...
  if (spec.kind === "silversea") {
    result = await spec.runSilverseaWeeklyBackgroundMaintenance({
      dryRun: true,
      triggerType,
      dispatchId: `p2-silversea-readonly-${Date.now()}`,
      supabaseClient: sb
    });
  } else if (spec.kind === "norwegian") {
    result = await spec.runNorwegianWeeklyBackgroundMaintenance({
      dryRun: true,
      triggerType,
      dispatchId: `p2-norwegian-readonly-${Date.now()}`,
      supabaseClient: sb
    });
  } else {
  ...
  const out = {
    ...
    ok: result.success !== false && result.ok !== false && result.blocked !== true,
    summary: result.summary || result.report || result
  };
  const file = path.join(root, "reports", `${line}-weekly-readonly-p2-pass${pass}-2026-09-06.json`);
