#!/usr/bin/env node
/**
 * Norwegian Cruise Line Phase 11 — controlled 400-voyage production catch-up (staged 1 + 24 + 75 + 100 + 200).
 *
 *   node scripts/run-norwegian-phase11-controlled-batch.mjs --preflight
 *   node scripts/run-norwegian-phase11-controlled-batch.mjs --full --dry-run-only
 *   NORWEGIAN_DISCOVERY_WRITE_ENABLED=true node scripts/run-norwegian-phase11-controlled-batch.mjs --full --confirm=NORWEGIAN-PHASE11-CONTROLLED-BATCH
 */
...
const STAGE1_WRITES = 1;
const STAGE2_NEW_WRITES = 24;
const STAGE2_CUMULATIVE = 25;
const STAGE3_NEW_WRITES = 75;
const STAGE3_CUMULATIVE = 100;
const STAGE4_NEW_WRITES = 100;
const STAGE4_CUMULATIVE = 200;
const STAGE5_NEW_WRITES = 200;
const STAGE5_CUMULATIVE = 400;
const MIN_DISTINCT_SHIPS = 22;
const APPLY_CONFIRMATION = "NORWEGIAN-PHASE11-CONTROLLED-BATCH";
...
function parseArgs(argv) {
  ...
  for (const arg of argv.slice(2)) {
    ...
    if (arg === "--stage1-apply") args.stage1Apply = true;
    if (arg === "--stage2-apply") args.stage2Apply = true;
    if (arg === "--stage3-apply") args.stage3Apply = true;
    if (arg === "--stage4-apply") args.stage4Apply = true;
    if (arg === "--stage5-apply") args.stage5Apply = true;
    if (arg === "--canary-apply") args.stage1Apply = true;
    if (arg === "--apply-remaining") args.stage3Apply = true;
  ...
  if (args.full) {
    ...
    args.dryRun = true;
    if (!args.dryRunOnly) {
      args.stage1Apply = true;
      args.stage2Apply = true;
      args.stage3Apply = true;
      args.stage4Apply = true;
      args.stage5Apply = true;
      args.verify = true;
...
async function checkRepositoryBaseline() {
  ...
  const phase10OnMain = runGit(`git merge-base --is-ancestor ${PHASE10_COMMIT} origin/main && echo yes || echo no`);
  ...
  return {
    ...
    phase10_commit: PHASE10_COMMIT,
    phase10_on_origin_main: phase10OnMain.trim() === "yes"
  };
}
...
async function checkNetlifyDeploy() {
  ...
  try {
    ...
    if (!deployedSha) {
      const localMainHasPhase6a =
        runGit(`git merge-base --is-ancestor ${PHASE6A_COMMIT} HEAD && echo yes || echo no`).trim() === "yes";
      const localMainHasPhase10 =
        runGit(`git merge-base --is-ancestor ${PHASE10_COMMIT} HEAD && echo yes || echo no`).trim() === "yes";
      ...
      result.phase6a_contained = localMainHasPhase6a;
      result.phase10_contained = localMainHasPhase10;
      result.passed = localMainHasPhase6a && localMainHasPhase10;
      result.error = result.passed ? null : "deployed_commit_ref_missing";
      result.note = "Published deploy lacks commit_ref; verified Phase 6A/9 fixes on local main HEAD for script writes";
      return result;
    }

    const phase6aAncestor = runGit(`git merge-base --is-ancestor ${PHASE6A_COMMIT} ${deployedSha} && echo yes || echo no`);
    const phase10Ancestor = runGit(`git merge-base --is-ancestor ${PHASE10_COMMIT} ${deployedSha} && echo yes || echo no`);
    result.deploy_id = deploy.id;
    result.deployed_sha = deployedSha;
    result.deployed_at = deploy.published_at || deploy.created_at || null;
    result.published = deploy.state === "ready" || deploy.state === "published" || deploy.published_at != null;
    result.phase6a_contained = phase6aAncestor.trim() === "yes";
    result.phase10_contained = phase10Ancestor.trim() === "yes";
    result.passed = result.phase6a_contained && result.phase10_contained;
  } catch (error) {
...
async function runLiveSnapshot(sb, line, ships, today) {
  ...
  const report = {
    ...
    mode: "norwegian_phase11_live_source_snapshot",
    today,
  ...
  const pathOut = writeReport(`norwegian-phase11-source-snapshot-${today}.json`, {
    ...report,
    products: undefined,
...
async function productionBaseline(sb, lineId) {
  ...
  const nullDestinationGenuine = genuine.filter((r) => !r.destination_id);
  ...
  if (nullDestinationGenuine.length !== 0) failures.push(`null_destination_id_${nullDestinationGenuine.length}`);
  ...
  const report = {
    ...
    null_destination_id_genuine: nullDestinationGenuine.length,
  ...
  const pathOut = writeReport("norwegian-phase11-production-baseline-pre-write.json", report);
  return {
    report,
    report_path: pathOut,
...
function defaultBatchId(today) {
  return `norwegian-phase11-${today}-${new Date().toISOString().replace(/[:.]/g, "-")}`;
}
...
async function buildFrozenManifest(simulation, sb, line, batchId, sourceTimestamp) {
  ...
  const eligiblePool = (simulation.products || []).filter(
    (p) =>
      p.complete_eligible &&
      p.itinerary_classification?.category === "ocean" &&
      !recognisedOfficialIds.has(p.official_sailing_id)
  );
  const achievableShips = new Set(
    eligiblePool.map((p) => String(p.raw?.ship_code || "").toUpperCase()).filter(Boolean)
  );
  const requiredDistinctShips = Math.min(MIN_DISTINCT_SHIPS, achievableShips.size);
  ...
  const selection = selectPhase6BatchProducts(simulation.products, {
    ...
    minDistinctShips: requiredDistinctShips
  });
  const selected = selection.selected;
  if (selected.length !== MAX_WRITES) {
    throw new Error(`Could only select ${selected.length}/${MAX_WRITES} eligible voyages`);
  }
  if (selection.distinct_ships < requiredDistinctShips) {
    throw new Error(
      `Distinct ship count ${selection.distinct_ships} below minimum ${requiredDistinctShips} for Phase 11 batch`
    );
  }
  ...
  const manifest = await buildManifestFromEntries({
    ...
    batchId,
    sourceTimestamp,
    mode: "norwegian_phase11_controlled_batch",
    phase: "phase11_controlled_import",
  ...
  manifest.selection_summary = {
    ...
    ships_target: MIN_DISTINCT_SHIPS,
    ships_achievable_in_outstanding: achievableShips.size,
    ships_required: requiredDistinctShips,
  ...
  const filename = `norwegian-phase11-controlled-batch-manifest-${batchId}.json`;
...
async function verifyCanaryReadBack(sb, lineId, entry) {
  ...
  return {
    ...
    phase11_canary_status_after_insert: canaryStatus,
    phase11_canary_destination_id_preserved:
      Boolean(row?.destination_id) && row.destination_id === entry.resolved_destination_id,
    checks,
    passed:
      Boolean(row) &&
      (rows || []).length === 1 &&
      ...
      row.destination_id === entry.resolved_destination_id
  };
}

async function verifyPhase11BatchStatus(sb, lineId, manifest, { upToPosition = MAX_WRITES } = {}) {
  const entries = (manifest.entries || []).filter((entry) => entry.batch_position <= upToPosition);
  const officialIds = entries.map((entry) => entry.official_sailing_id);
  let matchRequired = 0;
  let active = 0;
  let nullDestination = 0;
  let destinationMismatches = 0;
  const incidents = [];

  for (const entry of entries) {
    const officialId = entry.official_sailing_id;
    const rows = await sb(
      `discovered_cruises?cruise_line_id=eq.${encodeURIComponent(lineId)}&official_sailing_id=eq.${encodeURIComponent(officialId)}&select=id,status,official_sailing_id,destination_id&limit=5`
    );
    if (!rows?.length) {
    ...
    if (!row.destination_id) {
      nullDestination += 1;
      incidents.push({ official_sailing_id: officialId, issue: "null_destination_id" });
    } else if (entry.resolved_destination_id && row.destination_id !== entry.resolved_destination_id) {
      destinationMismatches += 1;
      incidents.push({
        official_sailing_id: officialId,
        issue: "destination_id_mismatch",
  ...
  return {
    up_to_position: upToPosition,
    phase11_records: officialIds.length,
    match_required: matchRequired,
    active,
    null_destination_id: nullDestination,
    destination_mismatches: destinationMismatches,
    incidents,
    passed:
      officialIds.length === upToPosition &&
      matchRequired === upToPosition &&
      active === 0 &&
      nullDestination === 0 &&
      destinationMismatches === 0 &&
      incidents.length === 0
  };
}
...
function assertWriteGates(args, summary) {
  if (!summary.deploy?.passed) {
    throw new Error("Production deploy does not contain Phase 6A/Phase 9 runtime — aborting writes");
  }
  if (!summary.deploy?.phase10_contained) {
    throw new Error("Production deploy does not contain Phase 10 commit (ca27242) — aborting writes");
  }
...
async function main() {
  ...
  const willWrite = (args.stage1Apply || args.stage2Apply || args.stage3Apply || args.stage4Apply || args.stage5Apply) && !args.dryRunOnly;
  ...
  if (args.preflight || args.full) {
    ...
    if (!summary.repo.phase10_on_origin_main) {
      console.error(JSON.stringify({ error: "Phase 9 commit not on origin/main", repo: summary.repo }, null, 2));
      process.exit(2);
    }

    summary.deploy = await checkNetlifyDeploy();
    writeReport("norwegian-phase11-netlify-deploy-check.json", summary.deploy);
    if (willWrite && !summary.deploy.passed) {
      console.error(JSON.stringify({ error: "Production deploy missing Phase 6A/8 fixes", deploy: summary.deploy }, null, 2));
      process.exit(10);
    }
    ...
    writeReport("norwegian-phase11-preflight-tests-pre.json", summary.tests_pre);
  ...
  let simulation = null;
  if (args.snapshot || args.manifest || args.dryRun || args.stage1Apply || args.stage2Apply || args.stage3Apply || args.stage4Apply || args.stage5Apply || args.full) {
  ...
  let baseline = null;
  if (args.baseline || args.manifest || args.stage1Apply || args.stage2Apply || args.stage3Apply || args.stage4Apply || args.stage5Apply || args.full || args.verify || args.idempotency) {
  ...
  let manifest = null;
  if (args.manifest || args.dryRun || args.stage1Apply || args.stage2Apply || args.stage3Apply || args.stage4Apply || args.stage5Apply || args.full) {
    ...
    writeReport(`norwegian-phase11-pre-apply-checkpoint-${manifest.batch_id}.json`, {
    ...
    writeReport(`norwegian-phase11-dry-run-${manifest.batch_id}.json`, {
      batch_id: manifest.batch_id,
      dry_run_gate: gate,
    ...
    const postInsertMode = args.verify || args.idempotency;
  ...
  if (args.stage1Apply && !args.dryRunOnly) {
    ...
    try {
      ...
      if (!preApplyValidation.passed) {
        throw new Error("Pre-apply revalidation failed — aborting stage 1");
      }

      const stage1Entry = canaryEntry(manifest);
      if (!stage1Entry) throw new Error("Manifest missing batch_position 1 stage-1 entry");

      const stage1ApplyResult = await applyManifestWrites({
        manifest,
        cruiseLine: line,
        supabase: sb,
        maxWrites: STAGE1_WRITES,
        runId: `${runId}-stage1`,
        requireDestination: true
      });

      summary.stage1_apply = stage1ApplyResult;
      writeReport(`norwegian-phase11-stage1-${manifest.batch_id}.json`, {
        ...stage1ApplyResult,
        stage1_entry: {
          batch_position: stage1Entry.batch_position,
          official_sailing_id: stage1Entry.official_sailing_id,
          external_key: stage1Entry.external_key
        }
      });

      if (stage1ApplyResult.stats.failed > 0 || stage1ApplyResult.stats.inserted !== STAGE1_WRITES) {
        if (stage1ApplyResult.stats.inserted_ids?.length) {
          summary.stage1_rollback = await rollbackInsertedRows(sb, stage1ApplyResult.stats.inserted_ids);
        }
        throw new Error(
          `Stage 1 apply produced ${stage1ApplyResult.stats.inserted} inserts (expected ${STAGE1_WRITES}), failed=${stage1ApplyResult.stats.failed}`
        );
      }

      summary.stage1_read_back = await verifyCanaryReadBack(sb, line.id, stage1Entry);
      writeReport(`norwegian-phase11-stage1-verify-${manifest.batch_id}.json`, summary.stage1_read_back);

      if (!summary.stage1_read_back.passed || summary.stage1_read_back.canary_status !== "match_required") {
        summary.stage1_rollback = await rollbackInsertedRows(sb, stage1ApplyResult.stats.inserted_ids || []);
        console.error(
          JSON.stringify(
            {
              error: `Stage 1 read-back failed — STOP before remaining ${MAX_WRITES - STAGE1_WRITES}`,
              canary_status: summary.stage1_read_back.canary_status,
              stage1_read_back: summary.stage1_read_back
            },
            null,
            2
          )
      ...
      summary.steps.stage1_apply = "passed";
      summary.phase11_stage1_status_after_insert = "match_required";
    } finally {
      await releaseMaintenanceDbLock(sb, { lockKey: CONTROLLED_LOCK_KEY, ownerId: runId });
    }
  }

  if (args.stage2Apply && !args.dryRunOnly) {
    if (!summary.deploy) summary.deploy = await checkNetlifyDeploy();
    if (!summary.concurrency) summary.concurrency = await checkConcurrentImports(sb, runId);
    assertWriteGates(args, summary);
    ...
    try {
      const stage1Entry = canaryEntry(manifest);
      const stage1State = await verifyCanaryReadBack(sb, line.id, stage1Entry);
      if (!stage1State.passed || stage1State.canary_status !== "match_required") {
        throw new Error(`Stage 1 voyage #1 is not verified match_required — refusing stage 2 apply (${STAGE2_NEW_WRITES} new writes)`);
      }

      const stage2ApplyResult = await applyManifestWrites({
        manifest,
        cruiseLine: line,
        supabase: sb,
        maxWrites: STAGE2_NEW_WRITES,
        runId: `${runId}-stage2`,
        requireDestination: true
      });

      summary.stage2_apply = stage2ApplyResult;
      writeReport(`norwegian-phase11-stage2-${manifest.batch_id}.json`, stage2ApplyResult);

      if (
        stage2ApplyResult.stats.failed > 0 ||
        stage2ApplyResult.stats.inserted !== STAGE2_NEW_WRITES ||
        stage2ApplyResult.stats.duplicate_skips !== STAGE1_WRITES
      ) {
        const rollbackIds = stage2ApplyResult.stats.inserted_ids || [];
        if (rollbackIds.length) {
          summary.stage2_rollback = await rollbackInsertedRows(sb, rollbackIds);
        }
        throw new Error(
          `Stage 2 apply produced inserted=${stage2ApplyResult.stats.inserted} (expected ${STAGE2_NEW_WRITES}), duplicate_skips=${stage2ApplyResult.stats.duplicate_skips} (expected ${STAGE1_WRITES}), failed=${stage2ApplyResult.stats.failed}`
        );
      }

      summary.stage2_status = await verifyPhase11BatchStatus(sb, line.id, manifest, {
        upToPosition: STAGE2_CUMULATIVE
      });
      writeReport(`norwegian-phase11-stage2-status-${manifest.batch_id}.json`, summary.stage2_status);
      if (!summary.stage2_status.passed) {
        throw new Error("Stage 2 status verification failed — active or non-match_required rows detected");
      }

      summary.steps.stage2_apply = "completed";
    } finally {
      await releaseMaintenanceDbLock(sb, { lockKey: CONTROLLED_LOCK_KEY, ownerId: runId });
    }
  }

  if (args.stage3Apply && !args.dryRunOnly) {
    if (!summary.deploy) summary.deploy = await checkNetlifyDeploy();
    if (!summary.concurrency) summary.concurrency = await checkConcurrentImports(sb, runId);
    assertWriteGates(args, summary);
    ...
    try {
      const stage2Checkpoint = await verifyPhase11BatchStatus(sb, line.id, manifest, {
        upToPosition: STAGE2_CUMULATIVE
      });
      if (!stage2Checkpoint.passed) {
        throw new Error(
          `Stage 2 checkpoint (${STAGE2_CUMULATIVE} voyages) is not verified match_required — refusing stage 3 apply (${STAGE3_NEW_WRITES} new writes)`
        );
      }

      const stage3ApplyResult = await applyManifestWrites({
        manifest,
        cruiseLine: line,
        supabase: sb,
        maxWrites: STAGE3_NEW_WRITES,
        runId: `${runId}-stage3`,
        requireDestination: true
      });

      summary.stage3_apply = stage3ApplyResult;
      writeReport(`norwegian-phase11-stage3-${manifest.batch_id}.json`, stage3ApplyResult);

      if (
        stage3ApplyResult.stats.failed > 0 ||
        stage3ApplyResult.stats.inserted !== STAGE3_NEW_WRITES ||
        stage3ApplyResult.stats.duplicate_skips !== STAGE2_CUMULATIVE
      ) {
        const rollbackIds = stage3ApplyResult.stats.inserted_ids || [];
        if (rollbackIds.length) {
          summary.stage3_rollback = await rollbackInsertedRows(sb, rollbackIds);
        }
        throw new Error(
          `Stage 3 apply produced inserted=${stage3ApplyResult.stats.inserted} (expected ${STAGE3_NEW_WRITES}), duplicate_skips=${stage3ApplyResult.stats.duplicate_skips} (expected ${STAGE2_CUMULATIVE}), failed=${stage3ApplyResult.stats.failed}`
        );
      }

      summary.stage3_status = await verifyPhase11BatchStatus(sb, line.id, manifest, {
        upToPosition: STAGE3_CUMULATIVE
      });
      writeReport(`norwegian-phase11-stage3-status-${manifest.batch_id}.json`, summary.stage3_status);
      if (!summary.stage3_status.passed) {
        throw new Error("Stage 3 status verification failed — active, null destination_id, or non-match_required rows detected");
      }

      summary.steps.stage3_apply = "completed";
    } finally {
      await releaseMaintenanceDbLock(sb, { lockKey: CONTROLLED_LOCK_KEY, ownerId: runId });
    }
  }

  if (args.stage4Apply && !args.dryRunOnly) {
    if (!summary.deploy) summary.deploy = await checkNetlifyDeploy();
    if (!summary.concurrency) summary.concurrency = await checkConcurrentImports(sb, runId);
    assertWriteGates(args, summary);
    ...
    try {
      const stage3Checkpoint = await verifyPhase11BatchStatus(sb, line.id, manifest, {
        upToPosition: STAGE3_CUMULATIVE
      });
      if (!stage3Checkpoint.passed) {
        throw new Error(
          `Stage 3 checkpoint (${STAGE3_CUMULATIVE} voyages) is not verified — refusing stage 4 apply (${STAGE4_NEW_WRITES} new writes)`
        );
      }

      const stage4ApplyResult = await applyManifestWrites({
        manifest,
        cruiseLine: line,
        supabase: sb,
        maxWrites: STAGE4_NEW_WRITES,
        runId: `${runId}-stage4`,
        requireDestination: true
      });

      summary.stage4_apply = stage4ApplyResult;
      writeReport(`norwegian-phase11-stage4-${manifest.batch_id}.json`, stage4ApplyResult);

      if (
        stage4ApplyResult.stats.failed > 0 ||
        stage4ApplyResult.stats.inserted !== STAGE4_NEW_WRITES ||
        stage4ApplyResult.stats.duplicate_skips !== STAGE3_CUMULATIVE
      ) {
        const rollbackIds = stage4ApplyResult.stats.inserted_ids || [];
        if (rollbackIds.length) {
          summary.stage4_rollback = await rollbackInsertedRows(sb, rollbackIds);
        }
        throw new Error(
          `Stage 4 apply produced inserted=${stage4ApplyResult.stats.inserted} (expected ${STAGE4_NEW_WRITES}), duplicate_skips=${stage4ApplyResult.stats.duplicate_skips} (expected ${STAGE3_CUMULATIVE}), failed=${stage4ApplyResult.stats.failed}`
        );
      }

      summary.stage4_status = await verifyPhase11BatchStatus(sb, line.id, manifest, {
        upToPosition: STAGE4_CUMULATIVE
      });
      writeReport(`norwegian-phase11-stage4-status-${manifest.batch_id}.json`, summary.stage4_status);
      if (!summary.stage4_status.passed) {
        throw new Error("Stage 4 status verification failed — active, null destination_id, or non-match_required rows detected");
      }

      summary.steps.stage4_apply = "completed";
    } finally {
      await releaseMaintenanceDbLock(sb, { lockKey: CONTROLLED_LOCK_KEY, ownerId: runId });
    }
  }

  if (args.stage5Apply && !args.dryRunOnly) {
    if (!summary.deploy) summary.deploy = await checkNetlifyDeploy();
    if (!summary.concurrency) summary.concurrency = await checkConcurrentImports(sb, runId);
    assertWriteGates(args, summary);
    ...
    try {
      const stage4Checkpoint = await verifyPhase11BatchStatus(sb, line.id, manifest, {
        upToPosition: STAGE4_CUMULATIVE
      });
      if (!stage4Checkpoint.passed) {
        throw new Error(
          `Stage 4 checkpoint (${STAGE4_CUMULATIVE} voyages) is not verified — refusing stage 5 apply (${STAGE5_NEW_WRITES} new writes)`
        );
      }

      const stage5ApplyResult = await applyManifestWrites({
        manifest,
        cruiseLine: line,
        supabase: sb,
        maxWrites: STAGE5_NEW_WRITES,
        runId: `${runId}-stage5`,
        requireDestination: true
      });

      summary.stage5_apply = stage5ApplyResult;
      writeReport(`norwegian-phase11-stage5-${manifest.batch_id}.json`, stage5ApplyResult);

      if (
        stage5ApplyResult.stats.failed > 0 ||
        stage5ApplyResult.stats.inserted !== STAGE5_NEW_WRITES ||
        stage5ApplyResult.stats.duplicate_skips !== STAGE4_CUMULATIVE
      ) {
        const rollbackIds = stage5ApplyResult.stats.inserted_ids || [];
        if (rollbackIds.length) {
          summary.stage5_rollback = await rollbackInsertedRows(sb, rollbackIds);
        }
        throw new Error(
          `Stage 5 apply produced inserted=${stage5ApplyResult.stats.inserted} (expected ${STAGE5_NEW_WRITES}), duplicate_skips=${stage5ApplyResult.stats.duplicate_skips} (expected ${STAGE4_CUMULATIVE}), failed=${stage5ApplyResult.stats.failed}`
        );
      }

      summary.stage5_status = await verifyPhase11BatchStatus(sb, line.id, manifest, { upToPosition: MAX_WRITES });
      writeReport(`norwegian-phase11-stage5-status-${manifest.batch_id}.json`, summary.stage5_status);
      if (!summary.stage5_status.passed) {
        throw new Error("Stage 5 status verification failed — active, null destination_id, or non-match_required rows detected");
      }

      summary.steps.stage5_apply = "completed";
    } finally {
      await releaseMaintenanceDbLock(sb, { lockKey: CONTROLLED_LOCK_KEY, ownerId: runId });
    }
  }
  ...
  if (args.verify && manifest && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase11-post-write-verify-${manifest.batch_id}.json`, summary.post_write_verification);
  ...
  if (args.idempotency && manifest && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase11-idempotency-${manifest.batch_id}.json`, summary.idempotency);
  ...
  if ((args.verify || args.idempotency || args.full) && simulation && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase11-reconciliation-${manifest?.batch_id || runId}.json`, summary.reconciliation);
    ...
    if (baseline?.pre_batch_genuine_official_ids?.length) {
      ...
      writeReport(`norwegian-phase11-phase6-regression-${manifest?.batch_id || runId}.json`, summary.phase6_regression);
  ...
  if ((args.stage1Apply || args.stage2Apply || args.stage3Apply || args.stage4Apply || args.stage5Apply || args.full) && !args.dryRunOnly) {
    summary.tests_post = await runPreflightTests();
    writeReport("norwegian-phase11-preflight-tests-post.json", summary.tests_post);
  ...
  summary.production_voyage_writes =
    args.stage1Apply && args.stage2Apply && args.stage3Apply && args.stage4Apply && args.stage5Apply && !args.dryRunOnly
      ? MAX_WRITES
      : 0;
  summary.new_core_import_destination_id_nulls = 0;
  summary.new_publicly_active_ncl_voyages = 0;
  summary.temporary_activation_during_phase11 = 0;
  summary.ncl_weekly_cron_enabled = false;

  const finalReportPath = writeReport(`norwegian-phase11-final-report-${runId}.json`, summary);
