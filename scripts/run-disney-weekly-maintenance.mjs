#!/usr/bin/env node
/**
 * Disney weekly maintenance — dry-run by default (Phase 5).
 *
 *   npm run disney:weekly-maintenance
 *   node scripts/run-disney-weekly-maintenance.mjs --today=2026-08-15
 *
 * Apply requires ALL of:
 *   --apply
 *   --confirm=DISNEY-WEEKLY-MAINTENANCE
 *   DISNEY_DISCOVERY_PRODUCTION_WRITES=true (process-scoped)
 *   local or self-hosted Mac execution
 *   --max-writes=<n> (hard-capped at DISNEY_MAX_WEEKLY_WRITES)
 */
...
const WEEKLY_APPLY_CONFIRMATION_TOKEN = "DISNEY-WEEKLY-MAINTENANCE";
const REPORT_DIR = path.join(root, "reports");

const { DISNEY_MAX_WEEKLY_WRITES } = require(path.join(
  root,
  "netlify/functions/lib/cruise-discovery-maintenance-runner"
));

export const MAX_WEEKLY_WRITES = DISNEY_MAX_WEEKLY_WRITES;
export { WEEKLY_APPLY_CONFIRMATION_TOKEN, DISNEY_LINE_SLUG };
...
export function isDisneyProductionWritesFlagEnabled(env = process.env) {
...
export function classifyExecutionEnvironment(env = process.env, { applyMode = false } = {}) {
  ...
  const runnerLabels = String(env.RUNNER_LABELS || "")
    ...
    .map((s) => s.trim())
    .filter(Boolean);
  const isSelfHosted = runnerLabels.includes("self-hosted") || runnerLabels.includes("disney-local-mac");
  const isCloudHosted =
...
export function assertWeeklyApplyAllowed(args, env = process.env) {
  ...
  if (!isDisneyProductionWritesFlagEnabled(env)) {
    throw makeError(
      "disney_weekly_production_writes_disabled",
      "Disney weekly maintenance is disabled (DISNEY_DISCOVERY_PRODUCTION_WRITES=false)"
    );
  }
...
export function buildWeeklyMaintenanceReport({
  ...
  const qg = summary.quality_gate || summary.source_quality_gate || {};
  ...
  const writesPerformed =
    mode === "dry_run"
      ...
      : (summary.writes_performed ??
        (summary.inserts || 0) + (summary.updates || 0) + (summary.source_absence_actions || 0));

  const inventoryUnchanged =
    mode === "dry_run"
      ? (countsBefore?.disney ?? 0) === (countsAfter?.disney ?? 0)
      : writesPerformed === 0 && (countsBefore?.disney ?? 0) === (countsAfter?.disney ?? 0);
  ...
  return {
    ...
    ended_at: endedAt,
    environment,
    line_slug: DISNEY_LINE_SLUG,
    snapshot_id: summary.snapshot_id || null,
    source: {
      num_found: summary.official_source_total ?? summary.source_total ?? null,
    ...
    quality_gates: {
      ...
      identity_gate:
        (rates.duplicate_official_identities || summary.source_quality_gate?.identity_collisions || 0) === 0 &&
        (rates.identity_coverage_pct ?? 100) >= 100
          ? "PASS"
          : "FAIL",
...
export function writeWeeklyMaintenanceReportFile(report, reportDir = REPORT_DIR) {
  ...
  const filePath = path.join(reportDir, `disney-weekly-maintenance-${stamp}.json`);
...
export function formatWeeklyMaintenanceSummary(report) {
  const qg =
    report.quality_gates?.source_quality_gate ||
    (report.source_quality_gate?.passed ? "PASS" : "FAIL");
  return [
    report.mode === "dry_run" ? "Disney weekly maintenance — DRY RUN" : "Disney weekly maintenance — APPLY",
...
async function main() {
  ...
  const { runDisneyWeeklyMaintenance } = require(path.join(
    root,
    "netlify/functions/lib/cruise-discovery-maintenance-runner"
  ));
  ...
  const line = (
    await sb(`ci_cruise_lines?slug=eq.${DISNEY_LINE_SLUG}&select=id,name,slug&limit=1`)
  )?.[0];
  ...
  const countsBefore = { disney: await activeCount() };
  ...
  const result = await runDisneyWeeklyMaintenance({
    ...
    maxWrites,
    today,
    runId: `disney-weekly-${startedAt.replace(/[:.]/g, "-")}`,
    supabase: sb,
    triggerType: args.apply ? "weekly_manual_apply" : "weekly_dry_run"
  });

  const countsAfter = { disney: await activeCount() };
