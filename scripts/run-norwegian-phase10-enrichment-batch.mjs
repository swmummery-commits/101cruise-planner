#!/usr/bin/env node
/**
 * Norwegian Cruise Line Phase 10 — controlled itinerary enrichment (50 voyages only).
 *
 *   node scripts/run-norwegian-phase10-enrichment-batch.mjs --dry-run --manifest=reports/norwegian-phase10-controlled-batch-manifest-....json
 *   NORWEGIAN_ENRICHMENT_WRITE_ENABLED=true node scripts/run-norwegian-phase10-enrichment-batch.mjs --apply --confirm=NORWEGIAN-PHASE10-ENRICHMENT --manifest=...
 */
...
const APPLY_CONFIRMATION = "NORWEGIAN-PHASE10-ENRICHMENT";
...
async function loadPopulation(sb, coreManifestPath) {
  ...
  const phase10 = JSON.parse(fs.readFileSync(resolvedPath, "utf8"));
  ...
  const matches = [];
  const missing = [];
  for (const entry of phase10.entries) {
  ...
  const extras = genuine.filter((r) => !phase10.entries.some((e) => e.official_sailing_id === r.official_sailing_id));
  ...
  return {
    ...
    manifest_count: phase10.entries.length,
...
async function main() {
  ...
  if (!args.manifestPath && !args.verifyOnly && !args.idempotencyOnly) {
    throw new Error("--manifest=path to Phase 10 core manifest is required");
  }
  const sb = createMaintenanceSupabase(root);
  const today = perthCalendarDate();
  const runId =
    args.runId || `norwegian-phase10-enrichment-${today}-${new Date().toISOString().replace(/[:.]/g, "-")}`;
  ...
  if (!summary.population.exact_match) {
    throw new Error(
      `Phase 10 identity set mismatch: missing=${summary.population.missing.length} count=${summary.population.matches.length}`
    );
  }
  ...
  const existingDryRunPath = args.runId
    ? path.join(REPORT_DIR, `norwegian-phase10-enrichment-dry-run-${args.runId}.json`)
    : null;
  ...
  if ((args.verifyOnly || args.idempotencyOnly) && existingDryRunPath && fs.existsSync(existingDryRunPath)) {
    ...
    const existingApplyPath = path.join(REPORT_DIR, `norwegian-phase10-enrichment-apply-${args.runId}.json`);
    ...
    summary.pre_write_snapshot_path = writeReport(`norwegian-phase10-enrichment-pre-write-snapshot-${today}.json`, preWriteSnapshot);

    const phase10EnrichmentManifest = {
    ...
    summary.phase10_enrichment_manifest_path = writeReport(
      `norwegian-phase10-enrichment-manifest-${runId}.json`,
      phase10EnrichmentManifest
    );
    ...
    summary.dry_run_path = writeReport(`norwegian-phase10-enrichment-dry-run-${runId}.json`, dryRunManifest);
    summary.dry_run = {
      page_stats: dryRunManifest.page_stats,
      port_totals: dryRunManifest.port_totals,
      outcome_counts: dryRunManifest.outcome_counts,
    ...
    if (args.apply || args.full) {
      ...
      try {
        ...
        summary.apply_path = writeReport(`norwegian-phase10-enrichment-apply-${runId}.json`, summary.apply);
  ...
  if (args.verify || args.full || args.verifyOnly) {
    ...
    summary.post_write_path = writeReport(`norwegian-phase10-enrichment-post-write-verify-${runId}.json`, summary.post_write);
  ...
  summary.final_report_path = writeReport(`norwegian-phase10-enrichment-final-report-${runId}.json`, summary);
