#!/usr/bin/env node
/**
 * Norwegian Cruise Line Phase 8 — third 50-voyage controlled production catch-up.
 *
 *   node scripts/run-norwegian-phase8-controlled-batch.mjs --preflight
 *   node scripts/run-norwegian-phase8-controlled-batch.mjs --full --dry-run-only
 *   NORWEGIAN_DISCOVERY_WRITE_ENABLED=true node scripts/run-norwegian-phase8-controlled-batch.mjs --full --canary-apply --apply-remaining --confirm=NORWEGIAN-PHASE8-CONTROLLED-BATCH
 */
...
const MAX_WRITES = 50;
const APPLY_CONFIRMATION = "NORWEGIAN-PHASE8-CONTROLLED-BATCH";
...
async function runLiveSnapshot(sb, line, ships, today) {
  ...
  const report = {
    ...
    mode: "norwegian_phase8_live_source_snapshot",
    today,
  ...
  const pathOut = writeReport(`norwegian-phase8-source-snapshot-${today}.json`, {
    ...report,
    products: undefined,
...
async function productionBaseline(sb, lineId) {
  ...
  const pathOut = writeReport("norwegian-phase8-production-baseline-pre-write.json", report);
  return {
    report,
    report_path: pathOut,
...
function defaultBatchId(today) {
  return `norwegian-phase8-${today}-${new Date().toISOString().replace(/[:.]/g, "-")}`;
}
...
async function buildFrozenManifest(simulation, sb, line, batchId, sourceTimestamp) {
  ...
  const manifest = await buildManifestFromEntries({
    ...
    batchId,
    sourceTimestamp,
    mode: "norwegian_phase8_controlled_batch",
    phase: "phase8_controlled_import",
  ...
  const filename = `norwegian-phase8-controlled-batch-manifest-${batchId}.json`;
...
async function verifyCanaryReadBack(sb, lineId, entry) {
  ...
  return {
    ...
    phase8_canary_status_after_insert: canaryStatus,
    checks,
...
async function verifyImmediatePhase7Status(sb, lineId, manifest) {
  ...
  return {
    phase8_records: officialIds.length,
...
async function main() {
  ...
  if (args.preflight || args.full) {
    ...
    writeReport("norwegian-phase8-netlify-deploy-check.json", summary.deploy);
    ...
    writeReport("norwegian-phase8-preflight-tests-pre.json", summary.tests_pre);
  ...
  if (args.manifest || args.dryRun || args.canaryApply || args.applyRemaining || args.full) {
    ...
    writeReport(`norwegian-phase8-pre-apply-checkpoint-${manifest.batch_id}.json`, {
    ...
    writeReport(`norwegian-phase8-dry-run-${manifest.batch_id}.json`, {
      batch_id: manifest.batch_id,
      dry_run_gate: gate,
  ...
  if (args.canaryApply && !args.dryRunOnly) {
    ...
    try {
      ...
      writeReport(`norwegian-phase8-canary-${manifest.batch_id}.json`, {
        ...canaryApply,
        canary_entry: {
      ...
      writeReport(`norwegian-phase8-canary-verify-${manifest.batch_id}.json`, summary.canary_read_back);
      ...
      summary.phase8_canary_status_after_insert = "match_required";
    } finally {
  ...
  if (args.applyRemaining && !args.dryRunOnly) {
    ...
    try {
      ...
      writeReport(`norwegian-phase8-apply-${manifest.batch_id}.json`, remainingApply);
      ...
      writeReport(`norwegian-phase8-immediate-status-${manifest.batch_id}.json`, summary.immediate_status);
      if (!summary.immediate_status.passed) {
        throw new Error("Immediate Phase 8 status verification failed — active or non-match_required rows detected");
      }
  ...
  if (args.verify && manifest && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase8-post-write-verify-${manifest.batch_id}.json`, summary.post_write_verification);
  ...
  if (args.idempotency && manifest && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase8-idempotency-${manifest.batch_id}.json`, summary.idempotency);
  ...
  if ((args.verify || args.idempotency || args.full) && simulation && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase8-reconciliation-${manifest?.batch_id || runId}.json`, summary.reconciliation);
    ...
    if (baseline?.pre_batch_genuine_official_ids?.length) {
      ...
      writeReport(`norwegian-phase8-phase6-regression-${manifest?.batch_id || runId}.json`, summary.phase6_regression);
  ...
  if ((args.canaryApply || args.applyRemaining || args.full) && !args.dryRunOnly) {
    ...
    writeReport("norwegian-phase8-preflight-tests-post.json", summary.tests_post);
  ...
  summary.temporary_activation_during_phase8 = 0;
  summary.ncl_weekly_cron_enabled = false;

  const finalReportPath = writeReport(`norwegian-phase8-final-report-${runId}.json`, summary);
