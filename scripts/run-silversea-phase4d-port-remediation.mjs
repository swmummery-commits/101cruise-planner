#!/usr/bin/env node
/**
 * Silversea Phase 4D — port remediation analysis and eligibility recheck.
 *
 *   node scripts/run-silversea-phase4d-port-remediation.mjs
 *   node scripts/run-silversea-phase4d-port-remediation.mjs --after-apply --write-frozen
 */
...
async function main() {
  ...
  const fullInventory = grouped.map((entry, index) => ({
    rank: index + 1,
    ...entry,
  ...
  const sailingDistribution = { gte5: 0, sailings_3_4: 0, sailings_2: 0, sailings_1: 0 };
  for (const row of fullInventory) {
    if (row.affected_sailings >= 5) sailingDistribution.gte5 += 1;
    else if (row.affected_sailings >= 3) sailingDistribution.sailings_3_4 += 1;
    else if (row.affected_sailings === 2) sailingDistribution.sailings_2 += 1;
    else sailingDistribution.sailings_1 += 1;
  }
  const expeditionRows = simulation.products.filter(
  ...
  const report = {
    ...
    eligibility,
    full_inventory_unresolved_analysis: fullInventory,
    remaining_identity_distribution: sailingDistribution,
    expedition_readiness: {
      total: expeditionRows.length,
      recognised_existing: expeditionRows.filter((row) =>
        existingByOfficialId.has(String(row.official_sailing_id || "").toUpperCase())
      ).length,
      itinerary_ports_unresolved: expeditionRows.filter((row) => (row.itinerary_ports_unresolved || 0) > 0).length
    },
    proposed_phase4d: {
      new_canonical_ports: PHASE4D_CANONICAL_PORT_CREATES,
      existing_port_aliases: PHASE4D_EXISTING_PORT_ALIASES,
      silversea_adapter_mappings: PHASE4D_SILVERSEA_ADAPTER_ALIASES
    },
    duration_mismatch_review: {
      funnel_count: eligibility.funnel.classic_duration_mismatch,
  ...
  const reportPath = path.join(REPORT_DIR, `silversea-phase4d-${suffix}-${stamp}.json`);
  fs.writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`);

  let frozenPath = null;
  if (WRITE_FROZEN && AFTER_APPLY && newEligibleCount > 0) {
    if (!fs.existsSync(FIXTURE_DIR)) fs.mkdirSync(FIXTURE_DIR, { recursive: true });
    frozenPath = path.join(FIXTURE_DIR, "phase4d-frozen-eligible.json");
    fs.writeFileSync(
      frozenPath,
      `${JSON.stringify(
        {
          phase: "phase4d_frozen_eligible",
