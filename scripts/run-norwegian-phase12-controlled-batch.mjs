#!/usr/bin/env node
/**
 * Norwegian Cruise Line Phase 12 — final bulk catch-up of all outstanding eligible ocean voyages.
 * Staged: 1 + 49 + 100 + 150 + 200 + 250 + remainder (dynamic target from reconciliation).
 *
 *   node scripts/run-norwegian-phase12-controlled-batch.mjs --preflight
 *   node scripts/run-norwegian-phase12-controlled-batch.mjs --full --dry-run-only
 *   NORWEGIAN_DISCOVERY_WRITE_ENABLED=true node scripts/run-norwegian-phase12-controlled-batch.mjs --full --confirm=NORWEGIAN-PHASE12-CONTROLLED-BATCH
 */
...
const PHASE11_COMMIT = "492610d99054c77a5314fa66230eb9febfac33c9";
const NETLIFY_SITE_ID = process.env.NETLIFY_SITE_ID || "ff34277c-6c91-4880-85b6-1240937c80eb";
const STAGE1_WRITES = 1;
const STAGE_THRESHOLDS = [
  { stage: 1, newWrites: 1 },
  { stage: 2, newWrites: 49 },
  { stage: 3, newWrites: 100 },
  { stage: 4, newWrites: 150 },
  { stage: 5, newWrites: 200 },
  { stage: 6, newWrites: 250 }
];
const APPLY_CONFIRMATION = "NORWEGIAN-PHASE12-CONTROLLED-BATCH";
const CONTROLLED_LOCK_KEY = `${LINE_SLUG}:phase12_controlled_batch`;

function buildStagePlan(target) {
  const plan = [];
  let prev = 0;
  for (const threshold of STAGE_THRESHOLDS) {
    if (prev >= target) break;
    const newWrites = Math.min(threshold.newWrites, target - prev);
    plan.push({
      stage: threshold.stage,
      newWrites,
      cumulative: prev + newWrites,
      prevCumulative: prev
    });
    prev += newWrites;
  }
  if (prev < target) {
    plan.push({
      stage: plan.length ? plan[plan.length - 1].stage + 1 : 1,
      newWrites: target - prev,
      cumulative: target,
      prevCumulative: prev
    });
  }
...
function parseArgs(argv) {
  ...
  for (const arg of argv.slice(2)) {
    ...
    if (arg === "--stage6-apply") args.stage6Apply = true;
    if (arg === "--stage7-apply") args.stage7Apply = true;
    if (arg === "--staged-apply") args.stagedApply = true;
...
async function checkRepositoryBaseline() {
  ...
  const phase11OnMain = runGit(`git merge-base --is-ancestor ${PHASE11_COMMIT} origin/main && echo yes || echo no`);
  ...
  return {
    ...
    phase11_commit: PHASE11_COMMIT,
    phase11_on_origin_main: phase11OnMain.trim() === "yes"
  };
}
...
async function checkNetlifyDeploy() {
  ...
  try {
    ...
    if (!deployedSha) {
      ...
      const localMainHasPhase11 =
        runGit(`git merge-base --is-ancestor ${PHASE11_COMMIT} HEAD && echo yes || echo no`).trim() === "yes";
      ...
      result.phase11_contained = localMainHasPhase11;
      result.passed = localMainHasPhase6a && localMainHasPhase10 && localMainHasPhase11;
    ...
    const phase11Ancestor = runGit(`git merge-base --is-ancestor ${PHASE11_COMMIT} ${deployedSha} && echo yes || echo no`);
    result.deploy_id = deploy.id;
    ...
    result.phase11_contained = phase11Ancestor.trim() === "yes";
    result.passed = result.phase6a_contained && result.phase10_contained && result.phase11_contained;
  } catch (error) {
...
async function runLiveSnapshot(sb, line, ships, today) {
  ...
  const report = {
    ...
    mode: "norwegian_phase12_live_source_snapshot",
    today,
  ...
  const pathOut = writeReport(`norwegian-phase12-source-snapshot-${today}.json`, {
    ...report,
    products: undefined,
...
async function productionBaseline(sb, lineId) {
  ...
  const pathOut = writeReport("norwegian-phase12-production-baseline-pre-write.json", report);
  return {
    report,
    report_path: pathOut,
...
function defaultBatchId(today) {
  return `norwegian-phase12-${today}-${new Date().toISOString().replace(/[:.]/g, "-")}`;
}
...
async function buildFrozenManifest(simulation, sb, line, batchId, sourceTimestamp) {
  ...
  const outstandingEligible =
    reconciliation?.reconciliation_arithmetic?.outstanding_eligible_inserts ??
    reconciliation?.source_recognition?.outstanding_eligible_inserts ??
    eligiblePool.length;

  const selection = selectAllOutstandingEligibleProducts(simulation.products, {
    excludeOfficialIds: recognisedOfficialIds
  });
  const selected = selection.selected;
  if (selected.length !== outstandingEligible) {
    throw new Error(
      `Selected ${selected.length} voyages but reconciliation outstanding eligible is ${outstandingEligible}`
    );
  }
  if (selected.length === 0) {
    throw new Error("No outstanding eligible voyages — catch-up already complete");
  }
  ...
  const manifest = await buildManifestFromEntries({
    ...
    batchId,
    sourceTimestamp,
    mode: "norwegian_phase12_final_bulk_batch",
    phase: "phase12_final_bulk_import",
    expectedCount: selected.length,
    requireDestination: true
  });

  manifest.phase12_target = selected.length;
  manifest.stage_plan = buildStagePlan(selected.length);
  manifest.selection_summary = {
    phase12_target: selected.length,
    outstanding_eligible: outstandingEligible,
    recognised_existing: recognisedOfficialIds.size,
  ...
  const filename = `norwegian-phase12-controlled-batch-manifest-${batchId}.json`;
...
async function verifyCanaryReadBack(sb, lineId, entry) {
  ...
  return {
    ...
    phase12_canary_status_after_insert: canaryStatus,
    phase12_canary_destination_id_preserved:
...
async function verifyPhase12BatchStatus(sb, lineId, manifest, { upToPosition } = {}) {
  const target = upToPosition ?? manifest.phase12_target ?? manifest.entries?.length ?? 0;
  const entries = (manifest.entries || []).filter((entry) => entry.batch_position <= target);
  ...
  return {
    ...
    phase12_records: officialIds.length,
    ...
    incidents,
    passed:
      officialIds.length === target &&
      matchRequired === target &&
      active === 0 &&
      nullDestination === 0 &&
...
async function postWriteVerification(sb, manifest, baselineLegacy) {
  const targetCount = manifest.phase12_target ?? manifest.entries?.length ?? 0;
  const indexes = await indexExistingNorwegianRecords(sb, manifest.cruise_line_id);
  const mismatches = [];
  const verified = [];
  ...
  return {
    ...
    passed: verified.length === targetCount && mismatches.length === 0 && legacyChanged.length === 0
  };
}
...
async function applyRemainingStages({ sb, line, manifest, runId, summary, args, stage1Entry }) {
  const target = manifest.phase12_target ?? manifest.entries?.length ?? 0;
  const stagePlan = manifest.stage_plan || buildStagePlan(target);
  summary.stage_plan = stagePlan;
  ...
  if (!stage1State.passed || stage1State.canary_status !== "match_required") {
    throw new Error("Stage 1 canary is not verified match_required — refusing remaining staged apply");
  }
  ...
  if (!lock.acquired) {
    throw new Error("Could not acquire Phase 12 controlled batch lock");
  }

  try {
    const remainingStages = stagePlan.filter((s) => s.stage > 1);
    for (const stage of remainingStages) {
      if (stage.prevCumulative > 0) {
        const checkpoint = await verifyPhase12BatchStatus(sb, line.id, manifest, {
          upToPosition: stage.prevCumulative
        });
        if (!checkpoint.passed) {
          throw new Error(
            `Stage ${stage.stage - 1} checkpoint (${stage.prevCumulative} voyages) failed — refusing stage ${stage.stage}`
          );
        }
      }
      ...
      const applyResult = await applyManifestWrites({
        ...
        cruiseLine: line,
        supabase: sb,
        maxWrites: stage.newWrites,
        runId: `${runId}-stage${stage.stage}`,
        requireDestination: true
      });

      summary[`stage${stage.stage}_apply`] = applyResult;
      writeReport(`norwegian-phase12-stage${stage.stage}-${manifest.batch_id}.json`, applyResult);

      if (
        applyResult.stats.failed > 0 ||
        applyResult.stats.inserted !== stage.newWrites ||
        applyResult.stats.duplicate_skips !== stage.prevCumulative
      ) {
        const rollbackIds = applyResult.stats.inserted_ids || [];
        if (rollbackIds.length) {
          summary[`stage${stage.stage}_rollback`] = await rollbackInsertedRows(sb, rollbackIds);
        }
        throw new Error(
          `Stage ${stage.stage} apply produced inserted=${applyResult.stats.inserted} (expected ${stage.newWrites}), duplicate_skips=${applyResult.stats.duplicate_skips} (expected ${stage.prevCumulative}), failed=${applyResult.stats.failed}`
        );
      }

      const stageStatus = await verifyPhase12BatchStatus(sb, line.id, manifest, {
        upToPosition: stage.cumulative
      });
      summary[`stage${stage.stage}_status`] = stageStatus;
      writeReport(`norwegian-phase12-stage${stage.stage}-status-${manifest.batch_id}.json`, stageStatus);
      if (!stageStatus.passed) {
        throw new Error(`Stage ${stage.stage} status verification failed`);
      }
      summary.steps[`stage${stage.stage}_apply`] = "completed";
    }
  } finally {
...
function assertWriteGates(args, summary) {
  ...
  if (!summary.deploy?.phase10_contained || !summary.deploy?.phase11_contained) {
    throw new Error("Production deploy does not contain Phase 10/11 runtime — aborting writes");
  }
...
async function main() {
  ...
  const willWrite = (args.stage1Apply || args.stagedApply) && !args.dryRunOnly;
  ...
  if (args.preflight || args.full) {
    ...
    if (!summary.repo.phase11_on_origin_main) {
      console.error(JSON.stringify({ error: "Phase 11 commit not on origin/main", repo: summary.repo }, null, 2));
      process.exit(2);
    }

    summary.deploy = await checkNetlifyDeploy();
    writeReport("norwegian-phase12-netlify-deploy-check.json", summary.deploy);
    ...
    writeReport("norwegian-phase12-preflight-tests-pre.json", summary.tests_pre);
  ...
  let simulation = null;
  if (args.snapshot || args.manifest || args.dryRun || args.stage1Apply || args.stagedApply || args.full) {
  ...
  let baseline = null;
  if (args.baseline || args.manifest || args.stage1Apply || args.stagedApply || args.full || args.verify || args.idempotency) {
  ...
  if (simulation) {
    const outstandingEligible =
      simulation?.production_reconciliation?.reconciliation_arithmetic?.outstanding_eligible_inserts ??
      simulation?.production_reconciliation?.source_recognition?.outstanding_eligible_inserts ??
      null;
    if (outstandingEligible === 0 && (args.manifest || args.full)) {
      summary.phase12_target = 0;
      summary.catch_up_already_complete = true;
      summary.reconciliation_pre = simulation.production_reconciliation;
      writeReport(`norwegian-phase12-already-complete-${runId}.json`, {
        run_id: runId,
        source_snapshot: summary.source_snapshot,
        production_baseline: baseline?.report || null,
        reconciliation: summary.reconciliation_pre,
        message: "Outstanding eligible = 0 — catch-up already complete; no Phase 12 writes required"
      });
      writeReport(`norwegian-phase12-final-summary-${runId}.json`, summary);
  ...
  let manifest = null;
  if (args.manifest || args.dryRun || args.stage1Apply || args.stagedApply || args.full) {
    ...
    writeReport(`norwegian-phase12-pre-apply-checkpoint-${manifest.batch_id}.json`, {
    ...
    writeReport(`norwegian-phase12-dry-run-${manifest.batch_id}.json`, {
      batch_id: manifest.batch_id,
      dry_run_gate: gate,
  ...
  if (args.stage1Apply && !args.dryRunOnly) {
    ...
    try {
      ...
      writeReport(`norwegian-phase12-stage1-${manifest.batch_id}.json`, {
        ...stage1ApplyResult,
        stage1_entry: {
      ...
      writeReport(`norwegian-phase12-stage1-verify-${manifest.batch_id}.json`, summary.stage1_read_back);
      ...
      if (!summary.stage1_read_back.passed || summary.stage1_read_back.canary_status !== "match_required") {
        ...
        console.error(
          JSON.stringify(
            {
              error: `Stage 1 read-back failed — STOP before remaining staged writes`,
      ...
      summary.phase12_stage1_status_after_insert = "match_required";
    } finally {
      await releaseMaintenanceDbLock(sb, { lockKey: CONTROLLED_LOCK_KEY, ownerId: runId });
    }
  }

  if (args.stagedApply && !args.dryRunOnly) {
    const stage1Entry = canaryEntry(manifest);
    await applyRemainingStages({ sb, line, manifest, runId, summary, args, stage1Entry });
    summary.steps.staged_apply = "completed";
  }
  ...
  if (args.verify && manifest && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase12-post-write-verify-${manifest.batch_id}.json`, summary.post_write_verification);
  ...
  if (args.idempotency && manifest && !args.dryRunOnly) {
    const phase12Target = manifest.phase12_target ?? manifest.entries?.length ?? 0;
    ...
    summary.idempotency = {
      ...
      passed:
        ...
        idemResult.stats.duplicate_skips === phase12Target &&
        afterCount === beforeCount
    };
    writeReport(`norwegian-phase12-idempotency-${manifest.batch_id}.json`, summary.idempotency);
  ...
  if ((args.verify || args.idempotency || args.full) && simulation && !args.dryRunOnly) {
    ...
    writeReport(`norwegian-phase12-reconciliation-${manifest?.batch_id || runId}.json`, summary.reconciliation);
    ...
    if (baseline?.pre_batch_genuine_official_ids?.length) {
      ...
      writeReport(`norwegian-phase12-phase6-regression-${manifest?.batch_id || runId}.json`, summary.phase6_regression);
  ...
  if ((args.stage1Apply || args.stagedApply || args.full) && !args.dryRunOnly) {
    summary.tests_post = await runPreflightTests();
    writeReport("norwegian-phase12-preflight-tests-post.json", summary.tests_post);
  ...
  summary.production_voyage_writes =
    args.stage1Apply && args.stagedApply && !args.dryRunOnly
      ? manifest?.phase12_target ?? manifest?.entries?.length ?? 0
      : 0;
  summary.new_core_import_destination_id_nulls = 0;
  summary.new_publicly_active_ncl_voyages = 0;
  summary.temporary_activation_during_phase12 = 0;
  summary.ncl_weekly_cron_enabled = false;

  const finalReportPath = writeReport(`norwegian-phase12-final-report-${runId}.json`, summary);
