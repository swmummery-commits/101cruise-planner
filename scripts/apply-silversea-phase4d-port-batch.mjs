#!/usr/bin/env node
/**
 * Silversea Phase 4D — apply canonical ports + existing-port alias/sync fixes.
 *
...
function runId() {
  return `silversea-phase4d-port-${new Date().toISOString().replace(/[:.]/g, "-")}`;
}
...
function planAliasWrites(csvText) {
  ...
  const writes = [];
  const skipped = [];
  for (const spec of PHASE4D_EXISTING_PORT_ALIASES) {
...
function resolveForVerification(alias, spec) {
  ...
  if (
    ...
    PHASE4D_SILVERSEA_ADAPTER_ALIASES.some((row) => row.source_label === alias)
  ) {
    resolution = adapter.resolveSilverseaPort(alias, "silversea_phase4d_verify");
  }
  return resolution;
}
...
function verifyAll(specs, aliasWrites) {
  ...
  for (const mapping of PHASE4D_SILVERSEA_ADAPTER_ALIASES) {
    const resolution = adapter.resolveSilverseaPort(mapping.source_label, "silversea_phase4d_verify");
    checks.push({
...
async function main() {
  ...
  for (const [index, spec] of PHASE4D_CANONICAL_PORT_CREATES.entries()) {
  ...
  if (proposed.length > 25) {
    throw new Error(`Proposed new canonical ports ${proposed.length} exceed Phase 4D limit of 25`);
  }
  ...
  if (APPLY && (aliasPlan.writes.length || proposed.length || csvSyncOnly.length)) {
    ...
    for (const spec of proposed) {
      const payload = {
        ...
        status: "verified",
        source: "silversea:phase4d_canonical_ports",
  ...
  if (APPLY) {
    report.verification = verifyAll(PHASE4D_CANONICAL_PORT_CREATES, aliasPlan.writes);
  ...
  const rollbackPath = path.join(REPORT_DIR, `silversea-phase4d-port-rollback-${id}.json`);
