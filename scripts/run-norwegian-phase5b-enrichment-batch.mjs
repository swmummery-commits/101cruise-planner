#!/usr/bin/env node
/**
 * Norwegian Cruise Line Phase 5B — controlled itinerary enrichment (25 voyages only).
 *
 *   node scripts/run-norwegian-phase5b-enrichment-batch.mjs --dry-run
 *   NORWEGIAN_ENRICHMENT_WRITE_ENABLED=true node scripts/run-norwegian-phase5b-enrichment-batch.mjs --apply --confirm=NORWEGIAN-PHASE5B-ENRICHMENT
 */
...
const APPLY_CONFIRMATION = "NORWEGIAN-PHASE5B-ENRICHMENT";
...
async function verifyPostWrite(sb, population, dryRunManifest, legacySnapshots) {
  ...
  return {
    ...
    passed: genuine.length === 25 && mismatches.length === 0 && legacyChanged.length === 0
  };
}
...
async function main() {
  ...
  const runId =
    args.runId || `norwegian-phase5b-${today}-${new Date().toISOString().replace(/[:.]/g, "-")}`;
  ...
  if (!summary.population.exact_match) {
    throw new Error(`Phase 4 identity set mismatch: missing=${summary.population.missing.length} extras=${summary.population.extras.length}`);
  }

  let dryRunManifest = null;
  const existingRunId = args.runId || "norwegian-phase5b-2026-08-13-2026-08-13T02-16-41-354Z";
  const existingDryRunPath = path.join(REPORT_DIR, `norwegian-phase5b-dry-run-${existingRunId}.json`);

  if (args.verifyOnly || args.idempotencyOnly) {
    if (!fs.existsSync(existingDryRunPath)) {
      throw new Error(`Existing dry-run manifest required: ${existingDryRunPath}`);
    }
    ...
    const existingApplyPath = path.join(REPORT_DIR, `norwegian-phase5b-apply-${existingRunId}.json`);
    ...
    summary.pre_write_snapshot_path = writeReport(`norwegian-phase5b-pre-write-snapshot-${today}.json`, preWriteSnapshot);

    const phase5bManifest = {
    ...
    summary.phase5b_manifest_path = writeReport(`norwegian-phase5b-enrichment-manifest-${runId}.json`, phase5bManifest);
    ...
    summary.dry_run_path = writeReport(`norwegian-phase5b-dry-run-${runId}.json`, dryRunManifest);
    summary.dry_run = {
      page_stats: dryRunManifest.page_stats,
      port_totals: dryRunManifest.port_totals,
      outcome_counts: dryRunManifest.outcome_counts,
    ...
    if (args.apply || args.full) {
      ...
      try {
        ...
        summary.apply_path = writeReport(`norwegian-phase5b-apply-${runId}.json`, summary.apply);
  ...
  if (args.verify || args.full || args.verifyOnly) {
    ...
    summary.post_write_path = writeReport(`norwegian-phase5b-post-write-verify-${runId}.json`, summary.post_write);
  ...
  summary.final_report_path = writeReport(`norwegian-phase5b-final-report-${runId}.json`, summary);
