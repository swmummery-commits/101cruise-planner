...
const cclShips = [
  ...
  { id: "s-breeze", name: "Breeze", cruise_line_id: "ccl-line", official_line_ship_id: "BR" }
];
const shipAliases = [
...
const destinations = ccl.catalogueDestinations([
  { id: "dest-caribbean", slug: "caribbean", name: "Caribbean", status: "published", classification_enabled: true },
  {
    id: "dest-aunz",
...
assert(breeze?.ship_resolution?.resolved === true, "official ship code resolves after remediation");
assert(breeze?.ship_resolution?.method === "official_line_ship_id", "breeze via official code");
assert(getaway?.destination_resolution?.destinationId === "dest-aunz", "Getaway maps to Australia/New Zealand with canonical ID");
assert(getaway?.destination_resolution?.catalogueOnly !== true, "Getaway requires destination ID not catalogue-only");

const mexFixture = normalised.find((row) => row.raw.region_code === "MB" || row.raw.region_code === "MR");
if (mexFixture) {
  assert(mexFixture.destination_resolution?.destinationId === "dest-mex", "MB/MR resolve to Mexican Riviera destination ID");
}

resetPortsCache();
const london = ccl.resolveCclDeparturePort({
  departure_port_name: "London, England",
  departure_port_code: "LON",
  ports_to_display: ["Dover (London)", "Le Havre (Paris)"]
});
assert(london.status === "resolved" && london.canonicalPortName === "Dover", "LON resolves to Dover with itinerary evidence");
assert(
  ccl.resolveCclDeparturePort({ departure_port_name: "London, England", departure_port_code: "LON" }).status === "ambiguous",
  "London without Dover evidence stays ambiguous"
);

const mobile = ccl.resolveCclDeparturePort({
  departure_port_name: "Mobile, AL",
  departure_port_code: "MOB"
});
assert(mobile.status === "resolved" && mobile.canonicalPortName === "Mobile", "Mobile resolves via Carnival MOB mapping");

const trust = require(path.join(root, "netlify/functions/lib/carnival-structured-source-trust"));
const cclTrust = trust.evaluateCarnivalStructuredSourceTrust({
  structured_source: "ccl_cruisesearch_api",
  sailing_id: "12345",
  itinerary_code: "BAW",
  ship_code: "CQ",
  departure_date: "2026-09-06",
  nights: 7,
  raw_extract: { ccl_sailing_id: "12345", ccl_itinerary_code: "BAW", ccl_ship_code: "CQ" }
});
assert(cclTrust.trusted === true, "ccl_cruisesearch_api trusted");
assert(
  trust.evaluateCarnivalStructuredSourceTrust({ structured_source: "ccl_cruisesearch_api_v2" }).trusted === false,
  "near-match Carnival source not trusted"
);
assert(
  trust.evaluateCarnivalStructuredSourceTrust({
