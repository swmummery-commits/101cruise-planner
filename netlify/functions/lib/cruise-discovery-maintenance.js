...
function assertCclWeeklyMaintenanceEnabled() {
  if (!isCarnivalWeeklyReconciliationEnabled()) {
    const err = new Error(
      "Carnival weekly maintenance is disabled (CARNIVAL_WEEKLY_RECONCILIATION_ENABLED=false)"
    );
    err.code = "carnival_weekly_maintenance_disabled";
    throw err;
  }
}
...
function assertAzamaraWeeklyMaintenanceEnabled() {
  if (!isAzamaraWeeklyReconciliationEnabled()) {
    const err = new Error(
      "Azamara weekly maintenance is disabled (AZAMARA_WEEKLY_RECONCILIATION_ENABLED=false)"
    );
    err.code = "azamara_weekly_maintenance_disabled";
    throw err;
  }
}
...
function describeMaintenanceHold() {
  return {
    ...
    bulk_import_flags_must_remain_false: [
      ...
      "AZAMARA_DISCOVERY_WRITE_ENABLED"
    ]
  };
}
