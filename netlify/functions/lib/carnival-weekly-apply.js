/**
 * Carnival Cruise Line — weekly maintenance apply.
 */

const { applyCclBatchWrites } = require("./carnival-discovery-writes");
const { validateCclWeeklyManifest } = require("./carnival-weekly-manifest");
const {
...
const { computeManifestHash } = require("./carnival-controlled-batch");
...
async function hideCclFromPublicInventory({ supabase, row, runId, perthToday, reason = "within_public_booking_cutoff" }) {
...
async function applyCclWeeklyManifest({
  manifest,
  supabase,
  cruiseLine,
  performWrites = true,
  ...
  const validation = validateCclWeeklyManifest(manifest);
  if (!validation.passed) {
    const err = new Error(`Carnival weekly manifest validation failed: ${validation.failures.join("; ")}`);
    err.code = "ccl_weekly_manifest_validation_failed";
  ...
  const actionableEntries = [...(manifest.inserts || []), ...(manifest.updates || [])].slice(0, maxWrites);
  const applyManifest = {
    generated_at: manifest.generated_at,
    mode: "carnival_weekly_maintenance_apply",
    run_id: runId,
    ...
    manifest_hash: computeManifestHash({
      mode: "carnival_weekly_maintenance_apply",
      cruise_line_id: cruiseLine.id,
      expected_record_count: actionableEntries.length,
      entries: actionableEntries
    })
  };
  ...
  if (!skipCutoffHides) {
    for (const hide of manifest.cutoff_hides || []) {
      ...
      try {
        const result = await hideCclFromPublicInventory({
          supabase,
          row,
          runId,
          perthToday,
