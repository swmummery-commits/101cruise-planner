/**
 * Royal Caribbean International Discovery execution modes and write safeguards.
 * Missing or invalid mode defaults to read-only — never write.
 * Prompt 2 keeps write flags OFF. Do not enable in this task.
 */
...
function assertRoyalCaribbeanWritesAllowed(modeGate) {
  if (modeGate?.writes_allowed) return;
  const err = new Error(
    modeGate?.reason === "production_write_flag_disabled"
      ? "Royal Caribbean Discovery production_write is disabled by ROYAL_CARIBBEAN_DISCOVERY_WRITE_ENABLED"
      : "Royal Caribbean Discovery writes are not permitted in this mode"
  );
  err.code = "royal_caribbean_discovery_write_forbidden";
