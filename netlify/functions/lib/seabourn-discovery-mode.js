...
function resolveSeabournDiscoveryMode(requestedMode) {
  ...
  if (mode === "weekly_maintenance") {
    const { isSeabournWeeklyReconciliationEnabled } = require("./cruise-discovery-maintenance");
    if (!isSeabournWeeklyReconciliationEnabled()) {
      return {
        mode,
        requested_mode: raw,
        writes_allowed: false,
        reason: "seabourn_weekly_reconciliation_disabled"
      };
    }
...
function assertSeabournWritesAllowed(modeGate) {
  if (modeGate?.writes_allowed) return;
  const err = new Error(
    modeGate?.reason === "production_write_flag_disabled"
      ? "Seabourn Discovery production_write is disabled by SEABOURN_DISCOVERY_WRITE_ENABLED"
      : "Seabourn Discovery writes are not permitted in this mode"
  );
  err.code = "seabourn_discovery_write_forbidden";
