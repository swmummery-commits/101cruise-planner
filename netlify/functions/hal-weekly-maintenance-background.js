/**
 * Holland America weekly inventory maintenance — Netlify Background Function.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.error("hal-weekly-maintenance-background failed", {
    ...
    return {
      ...
      body: JSON.stringify(
        redactSecrets({
          ...
          status: "failed",
          worker: BACKGROUND_FUNCTION_NAME,
          error: error.message || "HAL weekly background maintenance failed",
          code: error.code || null,
          elapsed_ms: Date.now() - started
        })
      )
