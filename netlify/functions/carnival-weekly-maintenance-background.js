/**
 * Carnival weekly inventory maintenance — Netlify Background Function.
 */

const { assertCronAuth, parseJsonBody, redactSecrets } = require("./lib/carnival-weekly-auth");
const {
  resolveDryRun,
  resolveMaxWrites,
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const result = await runCclWeeklyBackgroundMaintenance({
      dryRun,
      maxWrites,
      triggerType,
      dispatchId
    ...
    console.error("carnival-weekly-maintenance-background failed", {
    ...
    return {
      ...
      body: JSON.stringify(
        redactSecrets({
          ...
          status: "failed",
          worker: BACKGROUND_FUNCTION_NAME,
          error: error.message || "Carnival weekly background maintenance failed",
          code: error.code || null,
          elapsed_ms: Date.now() - started
        })
      )
