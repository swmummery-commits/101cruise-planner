/**
 * Azamara weekly inventory maintenance — Netlify Background Function.
 */

const { assertCronAuth, parseJsonBody, redactSecrets } = require("./lib/azamara-weekly-auth");
const {
  resolveDryRun,
  resolveMaxWrites,
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const result = await runAzamaraWeeklyBackgroundMaintenance({
      dryRun,
      maxWrites,
      triggerType,
      dispatchId
    ...
    console.error("azamara-weekly-maintenance-background failed", {
    ...
    return {
      ...
      body: JSON.stringify(
        redactSecrets({
          ...
          status: "failed",
          worker: BACKGROUND_FUNCTION_NAME,
          error: error.message || "Azamara weekly background maintenance failed",
          code: error.code || null,
          elapsed_ms: Date.now() - started
        })
      )
