/**
 * Disney weekly inventory maintenance — Netlify Background Function.
 */
...
const {
  ...
  parseJsonBody,
  redactSecrets
} = require("./lib/disney-weekly-auth");
const {
  resolveDryRun,
  resolveMaxWrites,
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const result = await runDisneyWeeklyBackgroundMaintenance({
      dryRun,
      maxWrites,
      triggerType,
      dispatchId
    ...
    console.error("disney-weekly-maintenance-background failed", {
    ...
    return {
      ...
      body: JSON.stringify(
        redactSecrets({
          ...
          status: "failed",
          worker: BACKGROUND_FUNCTION_NAME,
          error: error.message || "Disney weekly background maintenance failed",
          code: error.code || null,
          elapsed_ms: Date.now() - started
        })
      )
